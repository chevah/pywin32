 ...
 */
CPythonDialogTemplate::CPythonDialogTemplate(LPCWSTR caption, DLGTEMPLATE *tmpl, WORD fontsize,
	LPCWSTR font, LPCWSTR menu, LPCWSTR wclass)
{
	LPCWSTR thefont = font ? font: L"MS Sans Serif";

	m_alloc = sizeof(DLGTEMPLATE) +
		(wcslen(caption)*sizeof(caption[0])) +
		(wcslen(thefont)*sizeof(thefont[0])) +
		20;
...
BOOL CPythonDialogTemplate::Add(LPCWSTR wclass, DLGITEMTEMPLATE *tmpl, LPCWSTR txt,
...
static BOOL ParseDlgItemList(CPythonDialogTemplate *dlg, PyObject *tmpl)
{
	...
	// @tupleitem 6|buffer|extraData|A byte string or buffer used as extra data for the control.  The value depends on the control.
	...
	if (obitem==NULL)
		return FALSE;
	if (!PyArg_ParseTuple(obitem, "OOH|(hhhh)kkO:DLGITEMTEMPLATE",
		&obwclass,
		&obcaption,
		&tpl.id,
		&tpl.x, &tpl.y, &tpl.cx, &tpl.cy,
