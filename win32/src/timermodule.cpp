...
py_timer_set_timer (PyObject * self, PyObject * args)
{
	...
	if (!PyArg_ParseTuple (args, "kO:set_timer",
		...
		&callback))		// @pyparm function|TimerFunc||Callback function.  Will be called with with 2 int args: (timer_id, time)
		return NULL;
	...
	if (PyDict_SetItem (timer_id_callback_map,
						py_timer_id,
						callback) == -1) {
		::KillTimer (NULL, timer_id);
		Py_DECREF(py_timer_id);
...
static struct PyMethodDef timer_functions[] = {
	...
	{"set_timer",		py_timer_set_timer,		METH_VARARGS, "int = set_timer(milliseconds, callback}\nCreates a timer that executes a callback function"},
