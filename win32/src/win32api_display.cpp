...
void PyDISPLAY_DEVICE::deallocFunc(PyObject *ob)
{
	delete (PyDISPLAY_DEVICE *)ob;
}

PDISPLAY_DEVICE PyDISPLAY_DEVICE::GetDISPLAY_DEVICE(void)
{
...
PyObject *PyWinObject_FromDISPLAY_DEVICE(PDISPLAY_DEVICE pDISPLAY_DEVICE)
{
	...
	if (pDISPLAY_DEVICE->cb>cb){
		PyErr_Format(PyExc_WindowsError,"DISPLAY_DEVICE structure of size %d greater than supported size of %d", pDISPLAY_DEVICE->cb, cb);
		return NULL;
		}
