...
struct PyMemberDef PySecBufferDesc::members[] = {
	{"Version",	T_ULONG,  OFF(secbufferdesc.ulVersion), 0, "Currently should always be SECBUFFER_VERSION"},
	{NULL}
};
...
PySecBufferDesc::PySecBufferDesc(ULONG ulVersion)
{
	...
	if (obBuffers==NULL)
		PyErr_Format(PyExc_MemoryError,"PySecBufferDesc: unable to allocate %d SecBuffer's (%d bytes)",
			max_buffers, bufsize);
	else
...
PySecBufferDesc::~PySecBufferDesc()
{
	...
	for (ULONG buf_ind=0;buf_ind<secbufferdesc.cBuffers;buf_ind++)
		Py_XDECREF(obBuffers[buf_ind]);
	if (obBuffers!=NULL)
		free(obBuffers);
}
...
void PySecBufferDesc::deallocFunc(PyObject *ob)
{
	delete (PySecBufferDesc *)ob;
}

PSecBufferDesc PySecBufferDesc::GetSecBufferDesc(void)
{
...
PyObject *PySecBufferDesc::append(PyObject *self, PyObject *args)
{
	...
	if (psecbufferdesc->cBuffers > This->max_buffers){
		psecbufferdesc->pBuffers=(PSecBuffer)realloc(psecbufferdesc->pBuffers,psecbufferdesc->cBuffers*sizeof(SecBuffer));
...
void PySecBuffer::deallocFunc(PyObject *ob)
{
	delete (PySecBuffer *)ob;
}

PSecBuffer PySecBuffer::GetSecBuffer(void)
{
	return &secbuffer;
}
...
PyObject *PyWinObject_FromSecBuffer(PSecBuffer psecbuffer)
{
	return new PySecBuffer(psecbuffer);
}
...
struct PyMethodDef PyCtxtHandle::methods[] = {
	...
	{"CompleteAuthToken", PyCtxtHandle::CompleteAuthToken,1},	//@pymeth CompleteAuthToken|Completes the authentication token
	{"QueryContextAttributes", PyCtxtHandle::QueryContextAttributes, 1}, // @pymeth QueryContextAttributes|Retrieves info about a security context
	{"DeleteSecurityContext", PyCtxtHandle::DeleteSecurityContext, 1}, // @pymeth DeleteSecurityContext|Frees the security context and invalidates the handle
	{"QuerySecurityContextToken", PyCtxtHandle::QuerySecurityContextToken ,1}, // @pymeth QuerySecurityContextToken|Returns the access token for a security context
	{"MakeSignature", PyCtxtHandle::MakeSignature, 1}, // @pymeth MakeSignature|Generates a signature for a message
	{"VerifySignature", PyCtxtHandle::VerifySignature, 1}, // @pymeth VerifySignature|Verifies  a signature created using <om PyCtxtHandle.MakeSignature>
	{"EncryptMessage", PyCtxtHandle::EncryptMessage, 1}, // @pymeth EncryptMessage|Encrypts data with security context's session key
	{"DecryptMessage", PyCtxtHandle::DecryptMessage, 1}, // @pymeth DecryptMessage|Decrypts data encrypted by <om PyCtxtHandle.EncryptMessage>
	{"ImpersonateSecurityContext", PyCtxtHandle::ImpersonateSecurityContext, 1}, // @pymeth ImpersonateSecurityContext|Causes a server to act in the security context of an authenticated client
	{"RevertSecurityContext", PyCtxtHandle::RevertSecurityContext, 1}, // @pymeth RevertSecurityContext|Stops impersonation of a client initiated by <om PyCtxtHandle::ImpersonateSecurityContext>
	{NULL}
};
...
void PyCtxtHandle::deallocFunc(PyObject *ob)
{
	delete (PyCtxtHandle *)ob;
}

PCtxtHandle PyCtxtHandle::GetCtxtHandle(void)
{
	return &ctxthandle;
}

PyObject *PyCtxtHandle::tp_new(PyTypeObject *typ, PyObject *args, PyObject *kwargs)
{
	return new PyCtxtHandle();
}
...
PyObject *PyWinObject_FromCtxtHandle(PCtxtHandle pctxthandle)
{
	return new PyCtxtHandle(pctxthandle);
}

// @pymethod |PyCtxtHandle|MakeSignature|Creates a crytographic hash of a message using session key of the security context
PyObject *PyCtxtHandle::MakeSignature(PyObject *self, PyObject *args)
{
	...
	// @pyparm int|MessageSeqNo||A sequential number used by some packages to verify that no extraneous messages have been received
...
PyObject *PyCtxtHandle::VerifySignature(PyObject *self, PyObject *args)
{
	...
	// @comm The buffer configuration is dependent on the security package.  Usually there is a data buffer of type SECBUFFER_DATA
...
PyObject *PyCtxtHandle::CompleteAuthToken(PyObject *self, PyObject *args)
{
	...
	// @pyparm <o PySecBufferDesc>|Token||The buffer that contains the token buffer used when the context was initialized
...
PyObject *PyCtxtHandle::QueryContextAttributes(PyObject *self, PyObject *args)
{
	...
	switch (attr) {
		...
		case SECPKG_ATTR_CIPHER_STRENGTHS:
			...
			break;
		// @flag SECPKG_ATTR_CONNECTION_INFO|Returns a dictionary of connection info representing a SecPkgContext_ConnectionInfo struct
		case SECPKG_ATTR_CONNECTION_INFO:
			PSecPkgContext_ConnectionInfo ci;
			ci=(PSecPkgContext_ConnectionInfo)&buf;
			ret=Py_BuildValue("{s:l,s:l,s:l,s:l,s:l,s:l,s:l}",
				"Protocol",ci->dwProtocol,
				"Cipher",ci->aiCipher,
				"CipherStrength",ci->dwCipherStrength,
				"Hash",ci->aiHash,
				"HashStrength",ci->dwHashStrength,
				"Exch",ci->aiExch,
				"ExchStrength",ci->dwExchStrength);
			break;
		...
		case SECPKG_ATTR_SIZES:
			...
			ret=Py_BuildValue("{s:l,s:l,s:l,s:l}",
				"MaxToken",sz->cbMaxToken,
				"MaxSignature",sz->cbMaxSignature,
				"BlockSize",sz->cbBlockSize,
				"SecurityTrailer",sz->cbSecurityTrailer);
			break;
		...
		case SECPKG_ATTR_LIFESPAN:
			...
			ret=Py_BuildValue("NN",
				PyWinObject_FromTimeStamp(ls->tsStart),
				PyWinObject_FromTimeStamp(ls->tsExpiry));
			break;
		...
		case SECPKG_ATTR_NATIVE_NAMES:
			...
			ret=Py_BuildValue("NN",
				PyWinObject_FromWCHAR(nn->sClientName),
				PyWinObject_FromWCHAR(nn->sServerName));
		...
		case SECPKG_ATTR_STREAM_SIZES:
			...
			ret=Py_BuildValue("{s:l,s:l,s:l,s:l,s:l}",
				"Header",			ss->cbHeader,
				"Trailer",			ss->cbTrailer,
				"MaximumMessage",	ss->cbMaximumMessage,
				"Buffers",			ss->cBuffers,
		...
		case SECPKG_ATTR_KEY_INFO:
			...
			ret=Py_BuildValue("{s:u,s:u,s:l,s:l,s:l}",
				"SignatureAlgorithmName",ki->sSignatureAlgorithmName,
				"EncryptAlgorithmName",ki->sEncryptAlgorithmName,
				"KeySize", ki->KeySize,
				"SignatureAlgorithm", ki->SignatureAlgorithm,
				"EncryptAlgorithm", ki->EncryptAlgorithm);
...
// @pymethod |PyCtxtHandle|RevertSecurityContext|Stops impersonation of client context (see <om PyCtxtHandle::ImpersonateSecurityContext>)
...
{
	return Py_BuildValue("{s:l,s:l,s:l,s:l,s:u,s:u}",
			"Capabilities", psecpkginfo->fCapabilities,
			"Version",		psecpkginfo->wVersion,
			"RPCID",		psecpkginfo->wRPCID,
			"MaxToken",		psecpkginfo->cbMaxToken,
			"Name",			psecpkginfo->Name,
			"Comment",		psecpkginfo->Comment);
}
...
void PyCredHandle::deallocFunc(PyObject *ob)
{
	delete (PyCredHandle *)ob;
}

PCredHandle PyCredHandle::GetCredHandle(void)
{
	return &credhandle;
}

PyObject *PyCredHandle::tp_new(PyTypeObject *typ, PyObject *args, PyObject *kwargs)
{
	return new PyCredHandle();
}
...
PyObject *PyWinObject_FromCredHandle(PCredHandle pcredhandle)
{
	return new PyCredHandle(pcredhandle);
}
