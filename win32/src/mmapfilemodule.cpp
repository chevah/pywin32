...
static PyObject *
mmapfile_resize_method (mmapfile_object * self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "L|KO", keywords,
		&new_mapping_size.QuadPart,		// @pyparm long|MaximumSize||New size for file mapping. Use a multiple of system page size (see <om win32api.GetSystemInfo>)
		&new_offset.QuadPart,	// @pyparm long|FileOffset|0|Offset into file mapping.  Must be multiple of allocation granularity.
	...
	*/
	...
	if (new_mapping_size.QuadPart){
		...
		self->map_handle = CreateFileMapping (self->file_handle,
			NULL,
			PAGE_READWRITE,
			new_mapping_size.HighPart,
			new_mapping_size.LowPart,
			self->tagname);
		if (self->map_handle == NULL)
