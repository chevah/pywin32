...
PyObject *PyWinObject_FromCRYPT_KEY_PROV_INFO(PCRYPT_KEY_PROV_INFO pckpi)
{
	...
	for (DWORD i=0; i<pckpi->cProvParam; i++){
		...
		kpp=Py_BuildValue("{s:k, s:k, s:N}",
			"Param", pckpi->rgProvParam[i].dwParam,
			"Flags", pckpi->rgProvParam[i].dwFlags,
			"Data", data);
		if (kpp==NULL){
			Py_DECREF(obProvParam);
			Py_DECREF(data);
	...
	return Py_BuildValue("{s:u, s:u, s:k, s:k, s:k, s:N}",
		"ContainerName", pckpi->pwszContainerName,
		"ProvName", pckpi->pwszProvName,
		"ProvType", pckpi->dwProvType,
		"Flags", pckpi->dwFlags,
		"KeySpec", pckpi->dwKeySpec,
		"ProvParam", obProvParam);
}
...
PyObject *PyWinObject_FromCERT_OTHER_NAME(PCERT_OTHER_NAME pcon)
{
	...
	*/
	return Py_BuildValue("{s:s,s:N}",
		"ObjId", pcon->pszObjId,
		"Value", PyString_FromStringAndSize((char *)pcon->Value.pbData, pcon->Value.cbData));
}
...
PyObject *PyWinObject_FromCERT_ALT_NAME_ENTRY(PCERT_ALT_NAME_ENTRY pcane)
{
	switch(pcane->dwAltNameChoice){
		...
		case CERT_ALT_NAME_OTHER_NAME:
			...
			return Py_BuildValue("kN", pcane->dwAltNameChoice,
				PyWinObject_FromCERT_OTHER_NAME(pcane->pOtherName));
		default:
			return PyErr_Format(PyExc_NotImplementedError, "CERT_ALT_NAME_ENTRY %d is not yet supported", pcane->dwAltNameChoice);
		}
}
...
PyObject *PyWinObject_FromCRYPT_ALGORITHM_IDENTIFIER(PCRYPT_ALGORITHM_IDENTIFIER pcai)
{
	...
	*/
	return Py_BuildValue("{s:s, s:N}",
		"ObjId", pcai->pszObjId,
		"Parameters", PyString_FromStringAndSize((char *)pcai->Parameters.pbData, pcai->Parameters.cbData));
}
...
BOOL PyWinObject_AsCRYPT_ALGORITHM_IDENTIFIER(PyObject *obcai, PCRYPT_ALGORITHM_IDENTIFIER pcai)
{
	...
	return PyArg_ParseTupleAndKeywords(dummy_tuple, obcai, "sz#:CRYPT_ALGORITHM_IDENTIFIER", cai_keys,
		&pcai->pszObjId,	// @prop str|ObjId|An szOID_* string identifying the algorithm
		&pcai->Parameters.pbData, &pcai->Parameters.cbData);	// @prop str|Parameters|Blob of binary data containing encoded parameters
}

PyObject *PyWinObject_FromCERT_PUBLIC_KEY_INFO(PCERT_PUBLIC_KEY_INFO pcpki){
	return Py_BuildValue("{s:N, s:N}",
		"Algorithm", PyWinObject_FromCRYPT_ALGORITHM_IDENTIFIER(&pcpki->Algorithm),
...
PyObject *PyWinObject_FromCRYPT_BIT_BLOB(PCRYPT_BIT_BLOB pcbb)
{
	return Py_BuildValue("{s:N,s:k}",
		"Data", PyString_FromStringAndSize((char *)pcbb->pbData, pcbb->cbData),
...
BOOL PyWinObject_AsCRYPT_BIT_BLOB(PyObject *obcbb, PCRYPT_BIT_BLOB pcbb)
{
	...
	return PyArg_ParseTupleAndKeywords(dummy_tuple, obcbb, "Ok:CRYPT_BIT_BLOB", cbb_keys,
...
PyObject *PyWinObject_FromCERT_RDN_ATTR(PCERT_RDN_ATTR pcra)
{
	...
	if (value==NULL)
		return NULL;
	return Py_BuildValue("{s:s, s:k, s:N}",
		"ObjId", pcra->pszObjId,
		"ValueType", pcra->dwValueType,
		"Value", value);
}
...
PyObject *PyWinObject_FromCRYPT_OID_INFO(PCCRYPT_OID_INFO oid_info)
{
	return Py_BuildValue("{s:s,s:u,s:k,s:k,s:N}", 
		...
		"Value", oid_info->dwValue,  // this is union, but all same size integers
		"ExtraInfo", PyString_FromStringAndSize((char *)oid_info->ExtraInfo.pbData, oid_info->ExtraInfo.cbData));
}
...
BOOL PyWinObject_AsCRYPT_DECRYPT_MESSAGE_PARA(PyObject *obcdmp, PCRYPT_DECRYPT_MESSAGE_PARA pcdmp)
{
	...
	if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obcdmp, "O|kk:CRYPT_DECRYPT_MESSAGE_PARA", cdmp_keys,
		&obcertstores,
		&pcdmp->dwMsgAndCertEncodingType,
		&pcdmp->dwFlags))
		return FALSE;
...
{
	static char *cemp_keys[]={"ContentEncryptionAlgorithm", "CryptProv", "EncryptionAuxInfo",
		"Flags", "InnerContentType", "MsgEncodingType", NULL};
	...
	if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obcemp, "O|OOkkk:CRYPT_DECRYPT_MESSAGE_PARA", cemp_keys,
		&obcai,						// @prop <o PyCRYPT_ALGORITHM_IDENTIFIER>|ContentEncryptionAlgorithm|Identifies the algorithm to be used
		&obcryptprov,				// @prop <o PyCRYPTPROV>|CryptProv|Optional. Handle to provider that will perform encryption, can be None for default provider
		&obauxinfo,					// @prop object|EncryptionAuxInfo|Optional. Extra info required by some CSP's.  Not supported yet, use only None
		&pcemp->dwFlags,			// @prop int|Flags|Optional.  Combination of CRYPT_MESSAGE_*_FLAG constants
		&pcemp->dwInnerContentType,	// @prop int|InnerContentType|Optional.  Only used if message to be encrypted is already encoded
		&pcemp->dwMsgEncodingType))	// @prop int|MsgEncodingType|Optional.  Defaults to X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING
		return FALSE;
...
// @object PyGetSignerCertificate|Callback used with CRYPT_VERIFY_MESSAGE_PARA to locate a certficate by issuer and serial nbr.
...
PCCERT_CONTEXT WINAPI PyLocateCertificate(
	void* pvGetArg,
	...
	HCERTSTORE hMsgCertStore)
{
	...
	TmpPyObject args=Py_BuildValue("OkNN",
		callback_arg,
		dwCertEncodingType,
		Py_BuildValue("{s:N, s:N}",
			"Issuer", PyString_FromStringAndSize((char *)pSignerId->Issuer.pbData, pSignerId->Issuer.cbData),
...
BOOL PyWinObject_AsCRYPT_VERIFY_MESSAGE_PARA(PyObject *obcvmp, PCRYPT_VERIFY_MESSAGE_PARA pcvmp)
{
	...
	if (!PyDict_Check(obcvmp)){
		PyErr_SetString(PyExc_TypeError, "Object used to construct CRYPT_VERIFY_MESSAGE_PARA structure must be a dict or None");
		return FALSE;
		}
	if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obcvmp, "|kOOO:CRYPT_VERIFY_MESSAGE_PARA", cvmp_keys,
		&pcvmp->dwMsgAndCertEncodingType,	// @prop int|MsgAndCertEncodingType|Encoding types, defaults to X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING
		&obhcryptprov,			// @prop <o PyCRYPTPROV>|CryptProv|CSP to be used to verify signature. Use None for default provider.
		&callback_function,	// @prop function|<o PyGetSignerCertificate>|Callback function that locates signer's certificate.
...
{
	static char *csmp_keys[]={"SigningCert", "HashAlgorithm", "HashAuxInfo", "MsgCert", "MsgCrl",
		"AuthAttr", "UnauthAttr",  "Flags", "InnerContentType", "MsgEncodingType", NULL};
	...
	if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obcsmp, "OO|OOOOOkkk:CRYPT_SIGN_MESSAGE_PARA", csmp_keys,
		&obSigningCert,		// @prop <o PyCERT_CONTEXT>|SigningCert|Certficate to be used to sign message
		&obHashAlgorithm,	// @prop <o PyCRYPT_ALGORITHM_IDENTIFIER>|HashAlgorithm|Algorithm to be used for signed hash
		&obHashAuxInfo,		// @prop None|HashAuxInfo|Optional.  Param is reserved, use only None.
		&obMsgCert,			// @prop (<o PyCERT_CONTEXT>,...)|MsgCert|Optional sequence of certificate to be included in the message.
		&obMsgCrl,			// @prop (CRL_CONTEXT,...)|MsgCrl|Optional. Sequence of certificate revocation lists. Not yet supported, use only None.
...
BOOL PyWinObject_AsCRYPT_ATTRIBUTE(PyObject *obca, PCRYPT_ATTRIBUTE pca)
{
	...
	if (!PyArg_ParseTupleAndKeywords(dummy_tuple, obca, "sO:CRYPT_ATTRIBUTE", ca_keys,
...
{
	if (pca!=NULL){
		for (DWORD i=0; i<attr_cnt; i++)
			PyWinObject_FreeCRYPT_ATTRIBUTE(&pca[i]);
		free(pca);
		}
}
...
PyObject *PyWinObject_FromCERT_KEY_ATTRIBUTES_INFO(PCERT_KEY_ATTRIBUTES_INFO pckai)
{
	...
	if (pckai->pPrivateKeyUsagePeriod==NULL){
		...
		obusageperiod=Py_BuildValue("{s:N, s:N}",
			"NotBefore",	PyWinObject_FromFILETIME(pckai->pPrivateKeyUsagePeriod->NotBefore),
	...
	return Py_BuildValue("{s:N, s:N, s:N}",
		"KeyId",	PyString_FromStringAndSize((char *)pckai->KeyId.pbData, pckai->KeyId.cbData),
...
PyObject *PyWinObject_FromCERT_BASIC_CONSTRAINTS_INFO(PCERT_BASIC_CONSTRAINTS_INFO pcbci)
{
	...
	for (DWORD i=0; i<pcbci->cSubtreesConstraint; i++){
		PyObject *nb=PyString_FromStringAndSize((char *)pcbci->rgSubtreesConstraint[i].pbData, pcbci->rgSubtreesConstraint[i].cbData);
		if (nb==NULL){
			Py_DECREF(sc);
			return NULL;
			}
	...
	return Py_BuildValue("{s:N, s:N, s:k, s:N}",
		"SubjectType", PyWinObject_FromCRYPT_BIT_BLOB(&pcbci->SubjectType),
		"fPathLenConstraint", PyBool_FromLong(pcbci->fPathLenConstraint),
		"PathLenConstraint", pcbci->dwPathLenConstraint,
		"SubtreesConstraint", sc);
}
...
PyObject *PyWinObject_FromCERT_POLICY_INFO(PCERT_POLICY_INFO pcpi)
{
	...
	return Py_BuildValue("{s:s, s:N}",
		"PolicyIdentifier", pcpi->pszPolicyIdentifier,
		"PolicyQualifier", quals);
}
...
{
	return Py_BuildValue("{s:N, s:N, s:N}",
		"KeyId", PyString_FromStringAndSize((char *)pcaki->KeyId.pbData, pcaki->KeyId.cbData),
		"CertIssuer", PyString_FromStringAndSize((char *)pcaki->CertIssuer.pbData, pcaki->CertIssuer.cbData),
