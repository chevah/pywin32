...
struct PyMemberDef PyCRYPTPROV::members[] = {
	{NULL}
};
...
PyObject *PyCRYPTPROV::getattro(PyObject *self, PyObject *obname)
{
	return PyObject_GenericGetAttr(self,obname);
}

PyCRYPTPROV::~PyCRYPTPROV(void)
{
	CryptReleaseContext(hcryptprov,0);
}

void PyCRYPTPROV::deallocFunc(PyObject *ob)
{
	delete (PyCRYPTPROV *)ob;
}
	...
	switch (dwParam){
		...
		case PP_ENUMALGS:{
			...
			while (CryptGetProvParam(hc, dwParam, (BYTE *)&prov_enumalgs, &dwDataLen, dwFlags)){
				obdata=Py_BuildValue("{s:I,s:k,s:N}",
					"Algid", prov_enumalgs.aiAlgid,
					"BitLen", prov_enumalgs.dwBitLen,
		...
		case PP_ENUMALGS_EX:{
			...
			while (CryptGetProvParam(hc, dwParam, (BYTE *)&prov_enumalgs_ex, &dwDataLen, dwFlags)){
				obdata=Py_BuildValue("{s:I,s:k,s:k,s:k,s:k,s:N,s:N}",
					"Algid", prov_enumalgs_ex.aiAlgid,
					"DefaultLen", prov_enumalgs_ex.dwDefaultLen,
					"MinLen", prov_enumalgs_ex.dwMinLen,
					"MaxLen", prov_enumalgs_ex.dwMaxLen,
					"Protocols", prov_enumalgs_ex.dwProtocols,
					...
					"LongName", PyWinCoreString_FromString(prov_enumalgs_ex.szLongName, prov_enumalgs_ex.dwLongNameLen-1));
				if ((obdata==NULL) || (PyList_Append(ret, obdata)==-1)){
					Py_XDECREF(obdata);
					break;
					}
...
PyObject *PyCRYPTPROV::PyCryptGetUserKey(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "k", keywords,
		&dwKeySpec))	// @pyparm int|KeySpec||AT_KEYEXCHANGE or AT_SIGNATURE (some providers may implement extra key specs)
		return NULL;
...
PyObject *PyCRYPTPROV::PyCryptCreateHash(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "I|Ok", keywords,
		...
		&obhcryptkey,	// @pyparm <o PyCRYPTKEY>|Key|None|Used only for keyed hashes (MAC or HMAC), use None otherwise
		&dwFlags))		// @pyparm int|Flags|0|Reserved, use 0 if passed in
		return NULL;
...
PyObject *PyCRYPTPROV::PyCryptExportPublicKeyInfo(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "k|k:CryptExportPublicKeyInfo", keywords,
		...
		&encoding))	// @pyparm int|CertEncodingType|X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING|Specifies encoding for exported key info
		return NULL;
...
PyObject *PyCRYPTPROV::PyCryptImportPublicKeyInfo(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|k:CryptImportPublicKeyInfo", keywords,
		&obinfo,	// @pyparm dict|Info||<o PyCERT_PUBLIC_KEY_INFO> dictionary as returned by <om PyCRYPTPROV::CryptExportPublicKeyInfo>
		&encoding))	// @pyparm int|CertEncodingType|X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING|Specifies encoding for exported key info
		return NULL;
