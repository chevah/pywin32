...
// @pymethod bytes|win32crypt|CryptProtectData|Encrypts data using a session key derived from current user's logon credentials
static PyObject *PyCryptProtectData(PyObject *self, PyObject *args, PyObject *kwargs)
{
	static char *keywords[]={"DataIn", "DataDescr", "OptionalEntropy",
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|OOOOk:CryptProtectData", keywords, 
		...
		&obPromptStruct, //@pyparm <o PyCRYPTPROTECT_PROMPTSTRUCT>|PromptStruct|None|Contains options for UI display during encryption and decryption, can be None
	...
	bsuccess = CryptProtectData(&DataIn, DataDescr, pOptionalEntropy, pReserved,
		pPromptStruct, Flags, &DataOut);
...
// @pymethod (str, bytes)|win32crypt|CryptUnprotectData|Decrypts data that was encrypted using <om win32crypt.CryptProtectData>.
...
{
	static char *keywords[]={"DataIn", "OptionalEntropy",
		"Reserved", "PromptStruct", "Flags", NULL};
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptUnprotectData(&DataIn, &DataDescr, pOptionalEntropy, pReserved,
		pPromptStruct, Flags, &DataOut);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
		PyWin_SetAPIError("CryptProtectData");
		...
		ret = Py_BuildValue("NN",
			PyWinObject_FromWCHAR(DataDescr),
			PyWinObject_FromDATA_BLOB(&DataOut));
		if (DataDescr)
			LocalFree(DataDescr);
		LocalFree(DataOut.pbData);
		}
...
static PyObject *PyCryptAcquireContext(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOkk:CryptAcquireContext", keywords,
		&obcontainer_name, &obprovider_name, &dwProvType, &dwFlags))
		return NULL;
...
// @pymethod [(<o PyUnicode>,int),...]|win32crypt|CryptEnumProviderTypes|Lists available local cryptographic provider types
...
static PyObject *PyCryptSetProviderEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Okk:CryptSetProviderEx", keywords,
		...
		&dwFlags))		// @pyparm int|Flags||CRYPT_MACHINE_DEFAULT or CRYPT_USER_DEFAULT.  Combine with CRYPT_DELETE_DEFAULT to remove default.
		return NULL;
...
// @pymethod <o PyUnicode>|win32crypt|CryptFindLocalizedName|Returns localized name for predefined system stores (Root, My, .Default, .LocalMachine)
...
static PyObject *PyCertEnumSystemStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (obSystemStoreLocationPara!=NULL){
		if (dwFlags & CERT_SYSTEM_STORE_RELOCATE_FLAG){
			...
			if (!PyWinObject_AsWCHAR(obSystemStoreLocationPara,(WCHAR **)&pvSystemStoreLocationPara,TRUE))
				return NULL;
		}
	ret=PyList_New(0);
	if (ret==NULL)
...
static PyObject *PyCertOpenStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	CERT_SYSTEM_STORE_RELOCATE_PARA cssrp;
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&kOkO:CertOpenStore", keywords,
		PyWinLong_AsVoidPtr, &StoreProvider,	// @pyparm int|StoreProvider||CERT_STORE_PROV_*, currently does not accept string provider names
		&dwEncodingType,		// @pyparm int|MsgAndCertEncodingType||Only used with CERT_STORE_PROV_MSG, CERT_STORE_PROV_PKCS7, and CERT_STORE_PROV_FILENAME.
...
static PyObject *PyCertOpenSystemStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	WCHAR *store_name;
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|O:CertOpenSystemStore", keywords,
		&obstore_name,	// @pyparm <o PyUnicode>|SubsystemProtocol||Name of store to open, will be created if it doesn't already exist
...
static PyObject *PyCertRegisterSystemStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Ok:CertRegisterSystemStore", keywords,
		&obSystemStore, //@pyparm <o PyUnicode>|SystemStore||string/unicode name of store to be registered, or a sequence of (PyHkey, unicode) representing a CERT_SYSTEM_STORE_RELOCATE_PARA struct
		&dwFlags))   // @pyparm int|Flags||One of the CERT_SYSTEM_STORE_* location constants, can also be combined with CERT_SYSTEM_STORE_RELOCATE_FLAG and CERT_STORE_CREATE_NEW_FLAG
		return NULL;
...
static PyObject *PyCertUnregisterSystemStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	void* pvSystemStore=NULL;
	PyObject *ret=NULL, *obSystemStore=NULL;
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Ok:CertUnregisterSystemStore", keywords,
		&obSystemStore,	//@pyparm <o PyUnicode>|SystemStore||Name of System store to be unregistered
		&dwFlags))		// @pyparm int|Flags||CERT_SYSTEM_STORE_RELOCATE_FLAG, CERT_STORE_DELETE_FLAG (CERT_SYSTEM_STORE_RELOCATE_FLAG  not supported yet)
		return NULL;
	if (!PyWinObject_AsWCHAR(obSystemStore, (WCHAR **)&pvSystemStore))
		return NULL;
...
static PyObject *PyCryptFindOIDInfo(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "kO|k", keywords,
		&keytype,	// @pyparm int|KeyType||One of CRYPT_OID_INFO_OID_KEY,CRYPT_OID_INFO_NAME_KEY,CRYPT_OID_INFO_ALGID_KEY,CRYPT_OID_INFO_SIGN_KEY
	...
	switch (keytype){
		...
		case CRYPT_OID_INFO_SIGN_KEY:
			if (!PyTuple_Check(obkey)){
				PyErr_SetString(PyExc_TypeError,"Key must be a tuple of 2 ints when KeyType is CRYPT_OID_INFO_SIGN_KEY");
				return NULL;
				}
...
// @pymethod object|win32crypt|CryptGetKeyIdentifierProperty|Retrieves a property from a certificate by its key indentifier
...
static PyObject *PyCryptGetKeyIdentifierProperty(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|kkO:CryptGetKeyIdentifierProperty", keywords,
		...
		&propid,	// @pyparm int|PropId|CERT_KEY_PROV_INFO_PROP_ID|Property identifier, one of the CERT_*_PROP_ID values
		&flags,		// @pyparm int|Flags|0|Use CRYPT_KEYID_MACHINE_FLAG for machine keyset. (CRYPT_KEYID_ALLOC_FLAG is always added to Flags)
		&obcomputername))	// @pyparm <o PyUnicode>|ComputerName|None|Name of remote computer, use None for local machine
		return NULL;
	// This flag lets system allocate buffer of sufficient size to be freed with LocalFree
	flags|=CRYPT_KEYID_ALLOC_FLAG;
	if (!PyWinObject_AsReadBuffer(obkeyid, (void **)&chb.pbData, &chb.cbData))
		return NULL;
...
BOOL WINAPI CryptEnumKeyIdentifierProperties_callback(
  const CRYPT_HASH_BLOB *key_id,
  DWORD dwFlags,
  void* pvReserved,
  void* pvArg,
  DWORD cProp,
  ...
  DWORD* rgcbData)
{
	...
	for (DWORD prop_index=0; prop_index<cProp; prop_index++){
		...
		prop=Py_BuildValue("{s:k,s:N}",
			"PropId", rgdwPropId[prop_index],
			"Data", prop_data);
		if (prop==NULL){
			Py_DECREF(props);
			Py_DECREF(prop_data);
	...
	ret_item=Py_BuildValue("{s:N, s:N}",
		"KeyIdentifier", PyString_FromStringAndSize((char *)key_id->pbData, key_id->cbData),
		"Props", props);
	if (ret_item==NULL){
		Py_DECREF(props);
		return FALSE;
...
static PyObject *PyCryptEnumKeyIdentifierProperties(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "|OkkO:CryptEnumKeyIdentifierProperties", keywords,
		...
		&flags,		// @pyparm int|Flags|0|Can be CRYPT_KEYID_MACHINE_FLAG to list keys for local machine, or remote machine if ComputerName is given
		&obcomputername))	// @pyparm <o PyUnicode>|ComputerName|None|Name of remote computer, use None for local machine
		return NULL;
	if (obkeyid!=Py_None){
		if (!PyWinObject_AsReadBuffer(obkeyid, (void **)&chb.pbData, &chb.cbData))
...
// @pymethod <o PyCERT_CONTEXT>|win32crypt|CertAddSerializedElementToStore|Imports a serialized Certificate context, CRL, or CTL
...
static PyObject *PyCertAddSerializedElementToStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	BYTE *buf;
	DWORD bufsize, flags=0, contexttype=CERT_STORE_CERTIFICATE_CONTEXT_FLAG;
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOk|kk:CertAddSerializedElementToStore", keywords,
		...
		&contexttype,		// @pyparm int|ContextTypeFlags|CERT_STORE_CERTIFICATE_CONTEXT_FLAG|One of CERT_STORE_*_CONTEXT_FLAG constants
		&flags))			// @pyparm int|Flags|0|Reserved, use only 0
		return NULL;
	if (!PyWinObject_AsCERTSTORE(obcertstore, &hcertstore, TRUE))
		return NULL;
	if (!PyWinObject_AsReadBuffer(obdata, (void **)&buf, &bufsize, FALSE))
		return NULL;

	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CertAddSerializedElementToStore(hcertstore, buf, bufsize, adddisposition,
		flags, contexttype, &contexttype_out, &context);
	Py_END_ALLOW_THREADS
...
//	<nl>CertStore:<o PyCERTSTORE>,		## Handle to certificate store containing all certficates in the object, may be None
//	<nl>Msg:<o PyCRYPTMSG>,				## If input doesn't contains a PKCS7 message, will be None
...
{
	static char *keywords[]={"ObjectType", "Object", "ExpectedContentTypeFlags", "ExpectedFormatTypeFlags", "Flags", NULL};
	void *input, *context;
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "kO|kkk:CryptQueryObject", keywords, 
		...
		&formattype,	// @pyparm int|ExpectedFormatTypeFlags|CERT_QUERY_FORMAT_FLAG_ALL|One of the CERT_QUERY_FORMAT_FLAG_* constants
		&flags))		// @pyparm int|Flags|0|Reserved, use only 0
		return NULL;
	switch (objecttype){
		case CERT_QUERY_OBJECT_BLOB:
			if (!PyWinObject_AsReadBuffer(obinput, (void **)&blob_input.pbData, &blob_input.cbData))
				return NULL;
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptQueryObject(objecttype, input, contenttype, formattype, flags,
		&encoding, &contenttypeout, &formattypeout, &hcertstore, &hcryptmsg, (const void **)&context);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
...
//	<nl>{MsgType:int},					&nbsp&nbsp##Type of message decoded, one of CMSG_DATA,CMSG_SIGNED,CMSG_ENVELOPED,CMSG_SIGNED_AND_ENVELOPED,CMSG_HASHED
//	<nl>InnerContentType:int,			&nbsp&nbsp##Type of decoded content returned, uses same set of values as MsgType.  CMSG_DATA indicates unencoded data.
...
// @comm Only one level of encoding is interpreted.  Some types of messages will need multiple calls to completely decode.
//	For example, to decode a message created by <om win32crypt.CryptSignAndEncryptMessage>, one pass with CMSG_ENVELOPED_FLAG
...
static PyObject *PyCryptDecodeMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|Okkkl:CryptDecodeMessage", keywords,
		...
		&obcvmp,			// @pyparm dict|VerifyPara|None|<o PyCRYPT_VERIFY_MESSAGE_PARA> containing signature verification parms
		&msg_type_flags,	// @pyparm int|MsgTypeFlags|CMSG_ALL_FLAGS|Combination of CMSG_DATA_FLAG, CMSG_SIGNED_FLAG, CMSG_ENVELOPED_FLAG, CMSG_SIGNED_AND_ENVELOPED_FLAG, or CMSG_HASHED_FLAG
		&signer_ind,		// @pyparm int|SignerIndex|0|Index of the signer to verify,  ignored if message is not signed.
		&prev_inner_type,	// @pyparm int|PrevInnerContentType|0|Content type returned from previous call, used during subsequent pass on a nested message
		&returndata))		// @pyparm boolean|ReturnData|True|Indicates if decoded data should be returned.
		return NULL;
	if (!PyWinObject_AsReadBuffer(obbuf, (void **)&input_buf, &input_bufsize, FALSE))
		return NULL;
	...
	if (!returndata)
		return Py_BuildValue("{s:k,s:k,s:O,s:N,s:N}",
			"MsgType", msg_type,
			"InnerContentType", inner_type,
			"Decoded", Py_None,
			"XchgCert", PyWinObject_FromCERT_CONTEXT(exchange_cert),
			"SignerCert", PyWinObject_FromCERT_CONTEXT(signer_cert));
	...
	if (!bsuccess){
		...
		ret=Py_BuildValue("{s:k,s:k,s:N,s:N,s:N}",
			"MsgType", msg_type,
			"InnerContentType", inner_type,
			"Decoded", PyString_FromStringAndSize((char *)output_buf, output_bufsize),
			"XchgCert", PyWinObject_FromCERT_CONTEXT(exchange_cert),
			"SignerCert", PyWinObject_FromCERT_CONTEXT(signer_cert));
...
static PyObject *PyCryptEncryptMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	BYTE *inputbuf, *outputbuf=NULL;
	DWORD input_bufsize, output_bufsize=0, recipient_cnt=0;
	PCCERT_CONTEXT *recipients=NULL;
	BOOL bsuccess;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOO:CryptEncryptMessage", keywords,
		&obcemp,		// @pyparm <o PyCRYPT_ENCRYPT_MESSAGE_PARA>|EncryptPara||Encryption parameters
		&obrecipients,	// @pyparm (<o PyCERT_CONTEXT>,...)|RecipientCert||Sequence of handles to recipients' certificates
		&obbuf))		// @pyparm buffer|ToBeEncrypted||Data to be encrypted
		return NULL;

	if (!PyWinObject_AsReadBuffer(obbuf, (void **)&inputbuf, &input_bufsize, FALSE))
		return NULL;
	...
	if (!bsuccess)
		...
		if (outputbuf==NULL)
			...
			if (!bsuccess)
				...
				ret=PyString_FromStringAndSize((char *)outputbuf, output_bufsize);
			}
		}
...
static PyObject *PyCryptDecryptMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	PyCRYPT_DECRYPT_MESSAGE_PARA cdmp;
	BYTE *input_buf, *output_buf=NULL;
	DWORD input_bufsize, output_bufsize=0;
	PCCERT_CONTEXT exchange_cert=NULL;
	BOOL bsuccess;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO:CryptDecryptMessage", keywords,
		&obcdmp,	// @pyparm <o PyCRYPT_DECRYPT_MESSAGE_PARA>|DecryptPara||Dictionary containing decryption parameters
	...
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptDecryptMessage(&cdmp, input_buf, input_bufsize, output_buf, &output_bufsize, NULL);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
	...
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptDecryptMessage(&cdmp, input_buf, input_bufsize, output_buf, &output_bufsize, &exchange_cert);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
...
static PyObject *PyCryptSignAndEncryptMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	PCCERT_CONTEXT *recipients=NULL;
	BYTE *input_buf, *output_buf=NULL;
	DWORD recipient_cnt=0, input_bufsize, output_bufsize=0;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOOO:CryptSignAndEncryptMessage", keywords,
		&obcsmp,		// @pyparm <o PyCRYPT_SIGN_MESSAGE_PARA>|SignPara||Message signing parameters
		&obcemp,		// @pyparm <o PyCRYPT_ENCRYPT_MESSAGE_PARA>|EncryptPara||Encryption parameters
		&obrecipients,	// @pyparm (<o PyCERT_CONTEXT>,...)|RecipientCert||Sequence of certificates of intended recipients
	...
	if (!(PyWinObject_AsCRYPT_SIGN_MESSAGE_PARA(obcsmp, &csmp)
		...
		goto cleanup;
	if (!PyWinObject_AsReadBuffer(obinput_buf, (void **)&input_buf, &input_bufsize))
		goto cleanup;

	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptSignAndEncryptMessage(&csmp, &cemp, recipient_cnt, recipients,
		input_buf, input_bufsize, output_buf, &output_bufsize);
	Py_END_ALLOW_THREADS
	if (!bsuccess){
...
// @pymethod (<o PyCERT_CONTEXT>, str)|win32crypt|CryptVerifyMessageSignature|Verifies the signature of an encoded message
...
static PyObject *PyCryptVerifyMessageSignature(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|kOl:CryptVerifyMessageSignature", keywords,
		...
		&obcvmp,		// @pyparm <o PyCRYPT_VERIFY_MESSAGE_PARA>|VerifyPara|None|Signature verification parameters, use None for defaults
	...
	if (!PyWinObject_AsCRYPT_VERIFY_MESSAGE_PARA(obcvmp, &cvmp))
		return NULL;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptVerifyMessageSignature(&cvmp, signer_ind, input_buf, input_bufsize,
	...
	if (output_buf==NULL)
		...
		Py_BEGIN_ALLOW_THREADS
		bsuccess = CryptVerifyMessageSignature(&cvmp, signer_ind, input_buf, input_bufsize,
			output_buf, &output_bufsize, NULL);
		Py_END_ALLOW_THREADS
		if (!bsuccess){
			// Callback may have already set an exception
			if (!PyErr_Occurred())
...
static PyObject *PyCryptGetMessageCertificates(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	BYTE *input_buf;
	DWORD input_bufsize, flags=0, encoding_type=X509_ASN_ENCODING|PKCS_7_ASN_ENCODING;
	HCERTSTORE hcertstore=NULL;
	HCRYPTPROV csp=NULL;
	PyObject *obbuf, *obcsp=Py_None;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|kOk:CryptGetMessageCertificates", keywords,
		&obbuf,			// @pyparm buffer|SignedBlob||Buffer containing a signed message
		&encoding_type,	// @pyparm int|MsgAndCertEncodingType|X509_ASN_ENCODING\|PKCS_7_ASN_ENCODING|Message and certificate encoding types
	...
	if (!PyWinObject_AsHCRYPTPROV(obcsp, &csp, TRUE))
		return NULL;
	Py_BEGIN_ALLOW_THREADS
	hcertstore=CryptGetMessageCertificates(encoding_type, csp, flags, input_buf, input_bufsize);
	Py_END_ALLOW_THREADS
	if (hcertstore==NULL)
...
static PyObject *PyCryptGetMessageSignerCount(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	BYTE *input_buf;
	DWORD input_bufsize, encoding_type=X509_ASN_ENCODING|PKCS_7_ASN_ENCODING;
	long signer_cnt;
	PyObject *obbuf;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|k:CryptGetMessageSignerCount", keywords,
		&obbuf,				// @pyparm buffer|SignedBlob||Buffer containing a signed message
		&encoding_type))	// @pyparm int|MsgEncodingType|X509_ASN_ENCODING\|PKCS_7_ASN_ENCODING|Message encoding type
		return NULL;
	if (!PyWinObject_AsReadBuffer(obbuf, (void **)&input_buf, &input_bufsize, FALSE))
		return NULL;
	Py_BEGIN_ALLOW_THREADS
	signer_cnt=CryptGetMessageSignerCount(encoding_type, input_buf, input_bufsize);
	Py_END_ALLOW_THREADS
	if (signer_cnt == -1)
...
static PyObject *PyCryptSignMessage(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|l:CryptSignMessage", keywords,
		&obcsmp,		// @pyparm <o PyCRYPT_SIGN_MESSAGE_PARA>|SignPara||Message signing parameters
		&obmsgs,		// @pyparm (str,...)|ToBeSigned||Sequence of strings containing message data.  Can only contain 1 string if DetachedSignature parm is False.
		&detached_sig))	// @pyparm boolean|DetachedSignature|False|If True, only the signature itself is encoded in output msg.
		return NULL;
	if (!PyWinObject_AsCRYPT_SIGN_MESSAGE_PARA(obcsmp, &csmp))
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptSignMessage(&csmp, detached_sig, msg_cnt, (const BYTE **)msgs, msg_sizes,
...
// @pymethod <o PyCERT_CONTEXT>|win32crypt|CryptVerifyDetachedMessageSignature|Verifies a signature that is encoded separately from the data
...
static PyObject *PyCryptVerifyDetachedMessageSignature(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	BYTE *sig;
	BYTE **msgs=NULL;
	DWORD signer_ind, msg_cnt=0, sig_size, *msg_sizes=NULL;
	PCCERT_CONTEXT signer_cert;

	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "kOO|O:CryptVerifyDetachedMessageSignature", keywords,
		...
		&obcvmp))		// @pyparm <o PyCRYPT_VERIFY_MESSAGE_PARA>|VerifyPara|None|Signature verification parameters, use None for defaults
		return NULL;
	if (!PyWinObject_AsReadBuffer(obsig, (void **)&sig, &sig_size, FALSE))
		return NULL;
	if (!PyWinObject_AsCRYPT_VERIFY_MESSAGE_PARA(obcvmp, &cvmp))
		return NULL;
	if (!PyWinObject_AsPBYTEArray(obmsgs, &msgs, &msg_sizes, &msg_cnt))
...
static PyObject *PyCryptDecryptAndVerifyMessageSignature(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|Ok:CryptDecryptAndVerifyMessageSignature", keywords,
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptDecryptAndVerifyMessageSignature(&cdmp, &cvmp, signer_ind, input_buf, input_bufsize,
	...
	if (!bsuccess)
		...
		ret=Py_BuildValue("{s:N,s:N,s:N}",
			"Decrypted", PyString_FromStringAndSize((char *)output_buf, output_bufsize),
			"XchgCert", PyWinObject_FromCERT_CONTEXT(exchange_cert),
			"SignerCert", PyWinObject_FromCERT_CONTEXT(signer_cert));
...
static PyObject *PyCryptEncodeObjectEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|kkO:CryptEncodeObjectEx", keywords,
		&obstructtype,	// @pyparm str/int|StructType||OID identifying type of data to be encoded, either szOID_* string or a numeric id
		&obstruct,		// @pyparm dict|StructInfo||Information to be encoded.  Contents of dict are dependent on StructType
		&flags,			// @pyparm int|Flags|0|Encoding options, combination of CRYPT_UNICODE_* constants.  CRYPT_ENCODE_ALLOC_FLAG is added to flags..
		&encoding,		// @pyparm int|CertEncodingType|X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING|Encoding types
	...
	if ((oid_is_str && (strcmp(structtype, szOID_ENHANCED_KEY_USAGE)==0)) ||	// @flag szOID_ENHANCED_KEY_USAGE|<o PyCTL_USAGE> (sequence of OID's)
	...
	else if ((oid_is_str && (strcmp(structtype, szOID_KEY_USAGE)==0)) ||		// @flag szOID_KEY_USAGE|<o PyCRYPT_BIT_BLOB>
	...
	cleanup:
	if ((oid_is_str && (strcmp(structtype, szOID_ENHANCED_KEY_USAGE)==0)) ||
		(structtype==X509_ENHANCED_KEY_USAGE))
...
static PyObject *PyCryptDecodeObjectEx(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|kkO:CryptDecodeObjectEx", keywords,
		&obstructtype,	// @pyparm str/int|StructType||An OID identifying the type of data to be decoded, can be either str or int
		&obencoded,		// @pyparm str|Encoded||String or buffer containing ASN encoded data
		&flags,			// @pyparm int|Flags|0|Encoding options, can be combination CRYPT_UNICODE_* constants.  CRYPT_ENCODE_ALLOC_FLAG is added to flags..
		&encoding,		// @pyparm int|CertEncodingType|X509_ASN_ENCODING combined with PKCS_7_ASN_ENCODING|Encoding types
	...
	if (!PyWinObject_AsOID(obstructtype, &structtype, &oid_is_str))
		return NULL;
	if (!PyWinObject_AsReadBuffer(obencoded, (void **)&input_buf, &input_bufsize))
		return NULL;
	if (obpara!=Py_None){
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptDecodeObjectEx(encoding, structtype, input_buf, input_bufsize, flags, NULL, &output_buf, &output_bufsize);
	Py_END_ALLOW_THREADS
	if (!bsuccess){
		PyWin_SetAPIError("CryptDecodeObjectEx");
		goto cleanup;
	...
	if ((oid_is_str && (strcmp(structtype, szOID_ENHANCED_KEY_USAGE)==0)) ||	// @flag szOID_ENHANCED_KEY_USAGE|Sequence of OIDs
		...
		ret=PyWinObject_FromCRYPT_BIT_BLOB((CRYPT_BIT_BLOB *)output_buf);
			...
			((strcmp(structtype, szOID_SUBJECT_ALT_NAME)==0) ||					// @flag szOID_SUBJECT_ALT_NAME|<o PyCERT_ALT_NAME_INFO>
			(strcmp(structtype, szOID_SUBJECT_ALT_NAME2)==0) ||					// @flag szOID_SUBJECT_ALT_NAME2|<o PyCERT_ALT_NAME_INFO>
			(strcmp(structtype, szOID_ISSUER_ALT_NAME)==0) ||					// @flag szOID_ISSUER_ALT_NAME|<o PyCERT_ALT_NAME_INFO>
			(strcmp(structtype, szOID_ISSUER_ALT_NAME2)==0) ||					// @flag szOID_ISSUER_ALT_NAME2|<o PyCERT_ALT_NAME_INFO>
			(strcmp(structtype, szOID_NEXT_UPDATE_LOCATION)==0)))	||			// @flag szOID_NEXT_UPDATE_LOCATION|<o PyCERT_ALT_NAME_INFO>
		...
		ret=PyWinObject_FromCERT_BASIC_CONSTRAINTS2_INFO((PCERT_BASIC_CONSTRAINTS2_INFO)output_buf);
			...
			((strcmp(structtype, szOID_CERT_POLICIES)==0) ||					// @flag szOID_CERT_POLICIES|Sequence of <o PyCERT_POLICY_INFO> objects
			(strcmp(structtype, szOID_APPLICATION_CERT_POLICIES)==0))) ||		// @flag szOID_APPLICATION_CERT_POLICIES|Sequence of <o PyCERT_POLICY_INFO> objects
...
static PyObject *PyCertNameToStr(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O|kk:CertNameToStr", keywords,
		&obname,	// @pyparm str|Name||String containing an encoded CERT_NAME_INFO, as used with certificate Issuer and Subject
		&strtype,	// @pyparm int|StrType|CERT_SIMPLE_NAME_STR|Type of string to format, one of CERT_SIMPLE_NAME_STR,CERT_OID_NAME_STR,CERT_X500_NAME_STR
		&encoding))	// @pyparm int|CertEncodingType|X509_ASN_ENCODING|Input encoding
		return NULL;
	if (!PyWinObject_AsReadBuffer(obname, (void **)&cnb.pbData, &cnb.cbData))
		return NULL;
...
{
	static char *keywords[]={"StructType", "Encoded", "FormatStrType", "CertEncodingType",
		"FormatType", "FormatStruct", NULL};
	BYTE *input_buf;
	void *output_buf=NULL;
	DWORD input_bufsize, output_bufsize=0;
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OO|kkkO:CryptFormatObject", keywords,
		&oboid		,	// @pyparm str/int|StructType||OID identifying the type of encoded data, one of the szOID_* strings or an integer OID
		&obencoded,		// @pyparm str|Encoded||String containing encoded data to be formatted
		&string_fmt,	// @pyparm int|FormatStrType|0|String formatting options, combination of CRYPT_FORMAT_STR_MULTI_LINE, CRYPT_FORMAT_STR_NO_HEX
		&encoding,		// @pyparm int|CertEncodingType|X509_ASN_ENCODING|Input encoding
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptFormatObject(encoding, fmt_type, string_fmt, fmt_struct, oid,
		input_buf, input_bufsize, output_buf, &output_bufsize);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
...
static PyObject *PyPFXImportCertStore(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOk:PFXImportCertStore", keywords,
		...
		&flags))		// @pyparm int|Flags||Allowed flags are CRYPT_EXPORTABLE,CRYPT_USER_PROTECTED,CRYPT_MACHINE_KEYSET, and CRYPT_USER_KEYSET
		return NULL;
...
static PyObject *PyPFXVerifyPassword(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwargs, "OOk:PFXVerifyPassword", keywords,
		&obinput_buf,	// @pyparm bytes|PFX||Buffer containing PKCS#12-formatted certificate(s)
		&obpassword,	// @pyparm str|Password||Password used to encrypt the data, may be None
		&flags))		// @pyparm int|Flags||Allowed flags are CRYPT_EXPORTABLE,CRYPT_USER_PROTECTED,CRYPT_MACHINE_KEYSET, and CRYPT_USER_KEYSET
		return NULL;
...
static PyObject *PyCryptBinaryToString(PyObject *self, PyObject *args, PyObject *kwargs)
{
	...
	WCHAR *output_buf=NULL;
	PyObject *obinput_buf;
	DWORD flags, input_size, output_size;
	...
	BOOL bsuccess;
	Py_BEGIN_ALLOW_THREADS
	bsuccess = CryptBinaryToString(input_buf, input_size, flags, output_buf, &output_size);
	Py_END_ALLOW_THREADS
	if (!bsuccess)
...
static struct PyMethodDef win32crypt_functions[] = {
	...
	//@pymeth CryptFindLocalizedName|Return localized name for predefined system stores (Root, My, .Default, .LocalMachine)
	...
	{"CryptVerifyDetachedMessageSignature", (PyCFunction)PyCryptVerifyDetachedMessageSignature, METH_VARARGS|METH_KEYWORDS},
	// @pymeth CryptDecryptAndVerifyMessageSignature|Decrypts and decodes a signed message, and verifies its signatures
	{"CryptDecryptAndVerifyMessageSignature", (PyCFunction)PyCryptDecryptAndVerifyMessageSignature, METH_VARARGS|METH_KEYWORDS},
...
{
	PYWIN_MODULE_INIT_PREPARE(win32crypt, win32crypt_functions,
				  "Support for Windows cryptography functions");

	if (PyType_Ready(&PyCRYPTPROVType) == -1
	  ||PyType_Ready(&PyCRYPTKEYType) == -1
	  ||PyType_Ready(&PyCRYPTHASHType) == -1
	  ||PyType_Ready(&PyCRYPTMSGType) == -1
	  ||PyType_Ready(&PyCERTSTOREType) == -1
	  ||PyType_Ready(&PyCERT_CONTEXTType) == -1
