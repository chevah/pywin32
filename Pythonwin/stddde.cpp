...
void CDDEItem::Create(const TCHAR* pszName)
{
    m_strName = pszName;
}
...
void CDDETopic::Create(const TCHAR* pszName)
{
    m_strName = pszName;
}
...
BOOL CDDETopic::Poke(UINT wFmt, const TCHAR* pszItem,
...
BOOL CDDEConv::AdviseData(UINT wFmt, const TCHAR* pszTopic, const TCHAR* pszItem,
                          void* pData, DWORD dwSize)
{
    return FALSE;
}
...
void *_unindirect(void *ptr) {
	...
	//
	if (look->jmp == 0xe9) ptr = (byte *)ptr + (int)look->reladr +sizeof(JMPENTRY) ;
	return ptr ;
}
...
BOOL CDDEServer::Create(const TCHAR* pszServiceName,
...
//CT BEGIN
	...
	if (pTheServerList != NULL) {
		...
		CDDEServer *pOther = pTheServerList ;
		while (pOther->pServers != pTheServerList)
			pOther = pOther->pServers ;
...
//CT BEGIN
    ...
    uiResult = ::DdeInitialize(&m_dwDDEInstance,
                               (PFNCALLBACK)(thisDynamicCallback),
                               dwFilterFlags,
                               0);
#else
    uiResult = ::DdeInitialize(&m_dwDDEInstance,
...
HDDEDATA CALLBACK CDDEServer::StdDDECallback(
											WORD wType,
                                            WORD wFmt,
                                            HCONV hConv,
                                            HSZ hsz1,
...
HDDEDATA CALLBACK CDDEServer::DynDDECallback(
											CDDEServer *pServ,
											WORD wType,
                                            WORD wFmt,
                                            HCONV hConv,
...
void CDDEServer::PostAdvise(CDDETopic* pTopic, CDDEItem* pItem)
{
    ...
    ::DdePostAdvise(m_dwDDEInstance,
                    ::DdeCreateStringHandle(m_dwDDEInstance,
                                            ...
                                            DDE_STRING_CODEPAGE),
                    ::DdeCreateStringHandle(m_dwDDEInstance,
                                            (TCHAR*)(const TCHAR*)pItem->m_strName,
                                            DDE_STRING_CODEPAGE));
