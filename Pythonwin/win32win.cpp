...
#define CHECK_HWND_VALID(pWnd) if (!::IsWindow((pWnd)->m_hWnd)) RETURN_ERR(szErrMsgBadHandle);
...
public:
	...
	int OnMouseActivate( CWnd *pWnd, UINT ht, UINT msg)
	{ return CWnd::OnMouseActivate(pWnd, ht, msg);}
...
{
	return (PyObject *) ui_assoc_object::make (UITypeFromCObject(pWnd),
										   pWnd)->GetGoodRet();
...
PyObject *
ui_window_create(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
...
PyObject *
ui_window_create_window(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
PyObject *
ui_window_create_window_ex(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
PyObject *
PyCWnd::GetForegroundWindow(PyObject *self, PyObject *args)
{
...
PyObject *
PyCWnd::GetFocus(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_child_window_from_point(PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_window_get_dlg_item_text(PyObject *self, PyObject *args)
{
	int id;
	if (!PyArg_ParseTuple(args, "i:GetDlgItemText", &id )) // @pyparm int|idControl||The Id of the control to be retrieved.
		return NULL;
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_window_get_dlg_item_int(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_window_get_parent_frame(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
...
static PyObject *
ui_window_get_last_active_popup(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
...
static PyObject *
ui_window_get_top_level_parent(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
...
static PyObject *
ui_window_get_top_level_frame(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
...
static PyObject *
ui_window_get_top_level_owner(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
...
static PyObject *
ui_window_get_style(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_window_get_ex_style(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_window_on_ctl_color(PyObject *self, PyObject *args)
{
	PyObject *obDC;
    PyObject *obControl;
    int nCtlColor;
...
static PyObject *
ui_window_on_paint(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_window_on_close(PyObject *self, PyObject *args)
{
	if (!PyArg_ParseTuple(args, ":OnPaint"))
		return NULL;
...
static PyObject *
ui_window_redraw_window(PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
// @pymethod (x,y) or (l, t, r, b)|PyCWnd|ClientToScreen|Converts the client coordinates of a given point on the display to screen coordinates.
static PyObject *
ui_window_client_to_screen(PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args,"(iiii):ClientToScreen",
		&rect.left, &rect.top, &rect.right, &rect.bottom))
	{
		PyErr_Clear();
		bHaveRect = FALSE;
		if (!PyArg_ParseTuple(args,"(ii):ClientToScreen",
			&pnt.x, &pnt.y))
		{
			return NULL;
		}
...
ui_window_screen_to_client(PyObject *self, PyObject *args)
{
		...
		&rect.left, &rect.top, &rect.right, &rect.bottom))
	{
		PyErr_Clear();
		bHaveRect = FALSE;
		if (!PyArg_ParseTuple(args,"(ii):ScreenToClient",
			&pnt.x, &pnt.y))
		{
			return NULL;
		}
...
static PyObject *
ui_window_center_window (PyObject *self, PyObject *args)
{
	...
	// @pyparm <o PyCWnd>|altwin|None|alternate window relative to which it will be centered (other than the parent window).
...
PyObject *
ui_window_pump_waiting_messages(PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
// @pymethod |PyCWnd|RepositionBars|Repositions the windows control bars.( UINT nIDFirst, UINT nIDLast, UINT nIDLeftOver, UINT nFlag = CWnd::reposDefault, LPRECT lpRectParam = NULL, LPCRECT lpRectClient = NULL, BOOL bStretch = TRUE );
PyObject *
...
static PyObject *
ui_window_on_nc_hit_test( PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_window_on_set_cursor( PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_window_on_mouse_activate( PyObject *self, PyObject *args)
{
	CWnd *pWnd = GetWndPtr(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_window_pre_create_window(PyObject *self, PyObject *args)
{
	WndHack *pWnd = (WndHack *)GetWndPtr(self);
	if (!pWnd)
		return NULL;
	...
	GUI_END_SAVE;
	if (!ok)RETURN_ERR("CWnd::PreCreateWindow failed");
...
PyObject *
ui_window_on_query_new_palette(PyObject *self, PyObject *args)
{
...
static struct PyMethodDef PyCWnd_methods[] = {
	...
	{"ChildWindowFromPoint",ui_child_window_from_point,	1},     // @pymeth ChildWindowFromPoint|Identifies the child window that contains the point
	...
	{"CreateWindowEx",		ui_window_create_window_ex, 1}, // @pymeth CreateWindowEx|Creates the actual window for the PyCWnd object using extended attributes.
	...
	{"GetDlgItemInt",		ui_window_get_dlg_item_int,		1}, // @pymeth GetDlgItemInt|Returns the integer value of a child window or control with the specified ID.
	{"GetDlgItemText",		ui_window_get_dlg_item_text,	1}, // @pymeth GetDlgItemText|Returns the text of child window or control with the specified ID.
	{"GetLastActivePopup",	ui_window_get_last_active_popup,1}, // @pymeth GetLastActivePopup|Identifies the most recently active pop-up window
	...
	{"GetTopLevelParent",	ui_window_get_top_level_parent,	1}, // @pymeth GetTopLevelParent|Get the top-level parent window.
	...
	{"IsWindow",            ui_window_is_window,            1}, // @pymeth IsWindow|determines whether the specified window handle identifies an existing window.
	...
	{"MapWindowPoints",     ui_window_map_window_points,    1}, // @pymeth MapWindowPoints|Converts (maps) a set of points from the coordinate space of the CWnd to the coordinate space of another window.
	...
	{"RedrawWindow",        ui_window_redraw_window,        1}, // @pymeth RedrawWindow|Updates the specified rectangle or region in the given window's client area.
	...
	{"RepositionBars",      ui_window_reposition_bars,      1}, // @pymeth RepositionBars|Repositions the control bars for the window.
	...
	{"UnlockWindowUpdate",  ui_unlock_window_update,        1}, // @pymeth UnLockWindowUpdate|Unlocks a window that was locked with LockWindowUpdate
...
static PyObject *
PyCFrameWnd_CreateWindow(PyObject *self, PyObject *args)
	{
	CFrameWnd *pFrame=GetFramePtr(self);
...
PyCFrameWnd_pre_create_window(PyObject *self, PyObject *args)
{
	...
	public:
		BOOL PreCreateWindowBase(CREATESTRUCT &cs) {
...
PyCFrameWnd_DockControlBar(PyObject *self, PyObject *args)
{
	...
	GUI_END_SAVE;
	if (rc==NULL) PyErr_SetString(ui_module_error, "DockControlBar caused exception.");
...
// @pymethod |PyCFrameWnd|EndModalState|Ends the frame window's modal state. Enables all of the windows disabled by <om PyCFrameWnd.BeginModalState>.
static PyObject *
...
static struct PyMethodDef PyCFrameWnd_methods[] = {
	...
	{"CreateWindow",        PyCFrameWnd_CreateWindow,1}, // @pymeth CreateWindow|Creates the underlying window for the object.
	{"EndModalState",	    PyCFrameWnd_EndModalState,	1}, // @pymeth EndModalState|Ends the frame window's modal state. Enables all of the windows disabled by <om PyCFrameWnd.BeginModalState>.
	...
	{"PreCreateWindow",     PyCFrameWnd_pre_create_window,1}, // @pymeth PreCreateWindow|Calls the underlying MFC PreCreateWindow method.
...
PyObject *
ui_mdi_frame_on_context_help(PyObject *self, PyObject *args)
{
	CMDIFrameWnd *pWnd = GetMDIFrame(self);
	if (!pWnd)
		return NULL;
...
// @pymethod (<o PyCMDIChildWnd>, int)|PyCMDIFrameWnd|MDIGetActive|Retrieves the current active MDI child window, along with a flag indicating whether the child window is maximized.
static PyObject *
ui_mdi_frame_window_mdi_get_active( PyObject *self, PyObject *args)
{
...
static PyObject *
ui_mdi_frame_window_mdi_next( PyObject *self, PyObject *args)
{
	CMDIFrameWnd *pFrame = GetMDIFrame(self);
	if (!pFrame)
		return NULL;
...
static PyObject *
ui_mdi_frame_pre_translate_message(PyObject *self, PyObject *args)
{
	...
	GUI_END_SAVE;
	return Py_BuildValue("i(iiiii(ii))", rc, msg->hwnd,msg->message,msg->wParam,msg->lParam,msg->time,msg->pt.x,msg->pt.y);
}

// @pymethod |PyCMDIFrameWnd|OnCommand|Calls the standard Python framework OnCommand handler
static PyObject *
ui_mdi_frame_on_command(PyObject *self, PyObject *args)
{
...
static PyObject *
ui_mdi_frame_on_close(PyObject *self, PyObject *args)
{
...
static struct PyMethodDef PyCMDIFrameWnd_methods[] = {
	...
	{"PreTranslateMessage", ui_mdi_frame_pre_translate_message, 1}, // @pymeth PreTranslateMessage|Calls the underlying MFC PreTranslateMessage method.
	{"OnCommand", ui_mdi_frame_on_command, 1}, // @pymeth OnCommand|Calls the standard Python framework OnCommand handler
	{"OnContextHelp", ui_mdi_frame_on_context_help, 1}, // @pymeth OnContextHelp|Calls the underlying MFC OnContextHelp method.
...
static PyObject *
ui_mdi_child_window_create_window(PyObject *self, PyObject *args)
{
	CPythonMDIChildWnd *pWnd = GetPythonFrame(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_mdi_child_window_mdi_activate_frame(PyObject *self, PyObject *args)
{
	CPythonMDIChildWnd *pWnd = GetPythonFrame(self);
	if (!pWnd)
		return NULL;
...
static PyObject *
ui_mdi_child_window_pre_translate_message(PyObject *self, PyObject *args)
{
	// @xref <vm PyCWnd.PreTranslateMessage>
	CPythonMDIChildWnd *pWnd = GetPythonFrame(self);
	if (!pWnd)
...
static PyObject *
ui_mdi_child_window_on_command(PyObject *self, PyObject *args)
{
	// @xref <vm PyCWnd.OnCommand>
	// @pyparm int|wparam||
	// @pyparm int|lparam||
...
static PyObject *
ui_mdi_child_window_on_close(PyObject *self, PyObject *args)
{
	// @xref <vm PyCWnd.OnClose>
...
static struct PyMethodDef PyCMDIChildWnd_methods[] = {
	...
	{"CreateWindow", ui_mdi_child_window_create_window, 1}, // @pymeth CreateWindow|Creates the actual window for the PyCWnd object.
	{"GetMDIFrame",ui_mdi_child_window_get_mdi_frame,1}, // @pymeth GetMDIFrame|Returns the MDI parent frame
	{"MDIActivate", ui_mdi_child_window_mdi_activate_frame, 1}, // @pymeth MDIActivate|Activates the MDI frame independent of the main frame.
	...
	{"PreTranslateMessage", ui_mdi_child_window_pre_translate_message, 1}, // @pymeth PreTranslateMessage|Calls the underlying MFC PreTranslateMessage method.
	{"OnCommand", ui_mdi_child_window_on_command, 1}, // @pymeth OnCommand|Calls the standard Python framework OnCommand handler
	{"OnClose", ui_mdi_child_window_on_close, 1}, // @pymeth OnClose|Calls the standard Python framework OnClose handler
	{NULL,			NULL}
};
