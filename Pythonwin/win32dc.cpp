...
{
	return (CDC *)GetGoodCppObject( self, &type);
}
...
static PyObject *
ui_dc_setpixel (PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
		return NULL;
...
static PyObject *
ui_dc_get_safe_hdc(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
...
static PyObject *
ui_dc_get_pixel (PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
		return NULL;
...
static PyObject *
ui_dc_get_current_position (PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
		return NULL;
...
static PyObject *
ui_dc_pie (PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
		return NULL;
...
PyObject *ui_dc_object::create_compatible_dc( PyObject *self, PyObject *args )
{
	...
	ui_dc_object *dc =
	       (ui_dc_object *) ui_assoc_object::make (ui_dc_object::type, pcDC)->GetGoodRet();
	return dc;
}
...
static PyObject *
ui_dc_draw3drect(PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC)
		return NULL;
	...
	if (!PyArg_ParseTuple (args, "(iiii)ii:Draw3dRect", 
	          ...
	          &cbr)) // @pyparm int|colorBotRight||Specifies the color of the bottom and right sides of the three-dimensional rectangle.
		return NULL;
	GUI_BGN_SAVE;
...
static PyObject *
ui_dc_get_map_mode (PyObject *self, PyObject *args)
{
  CDC *pDC = ui_dc_object::GetDC(self);
  if (!pDC)
	return NULL;
...
// @pymethod x, y|PyCDC|GetWindowOrg|Retrieves the x- and y-coordinates of the origin of the window associated with the device context.
static PyObject *
...
// @pymethod int|PyCDC|SetWorldTransform|sets a two-dimensional linear transformation between world space and page space for the specified device context. This transformation can be used to scale, rotate, shear, or translate graphics output.
static PyObject *
...
static PyObject *
ui_dc_draw_frame_control(PyObject *self, PyObject *args)
{
	CDC *pDC = ui_dc_object::GetDC(self);
	if (!pDC) return NULL;
...
static struct PyMethodDef ui_dc_methods[] = {
	...
	{"DrawFrameControl",    ui_dc_draw_frame_control,1}, // @pymeth DrawFrameControl|Draws a frame control of the specified type and style.
	...
	{"EndPath",				ui_dc_end_path,				1}, // @pymeth EndPath|Closes a path bracket and selects the path defined by the bracket into the specified device context
	{"ExtTextOut",          ui_dc_ext_text_out, 1}, //@pymeth ExtTextOut|Writes text to the DC.
	{"FillPath",			ui_dc_fill_path,			1}, // @pymeth FillPath|Closes any open figures in the current path and fills the path's interior by using the current brush and polygon-filling mode.
	...
	{"GetSafeHdc",          ui_dc_get_safe_hdc, 1}, // @pymeth GetSafeHdc|Returns the underlying windows handle for the DC object.
	...
	{"GetViewportExt",		ui_dc_get_viewport_ext,		1}, // @pymeth GetViewportExt|Gets the viewport extent of the device context
	{"GetViewportOrg",		ui_dc_get_viewport_org,		1}, // @pymeth GetViewportOrg|Gets the viewport origin of the device context
	{"GetWindowExt",		ui_dc_get_window_ext,		1}, // @pymeth GetWindowExt|Gets the window extent of the device context
	{"GetWindowOrg",		ui_dc_get_window_org,		1}, // @pymeth GetWindowOrg|Retrieves the x- and y-coordinates of the origin of the window associated with the device context.
	{"IntersectClipRect",   ui_dc_intersect_clip_rect,  1}, // @pymeth IntersectClipRect|Creates a new clipping region by forming the intersection of the current region and the rectangle specified
	...
	{"OffsetWindowOrg",     ui_dc_offset_window_org,1}, // @pymeth OffsetWindowOrg|Modifies the coordinates of the window origin relative to the coordinates of the current window origin.
	{"OffsetViewportOrg",   ui_dc_offset_viewport_org,1}, // @pymeth OffsetViewportOrg|Modifies the coordinates of the viewport origin relative to the coordinates of the current viewport origin
	...
	{"SetGraphicsMode",     ui_dc_set_graphics_mode, 1}, // @pymeth SetGraphicsMode|Sets the graphics mode for the specified device context
	...
	{"SetViewportOrg",		ui_dc_set_viewport_org,		1}, // @pymeth SetViewportOrg|Sets the viewport origin of the device context
