...
CDockContext *PyCDockContext::GetDockContext (PyObject *self)
{
	return (CDockContext *)GetGoodCppObject( self, &type);
}
...
PyObject *
PyCControlBar::create (PyObject *self, PyObject *args)
{
...
{
	return (CControlBar *)GetGoodCppObject( self, &type);
}
...
PyObject *PyCControlBar_CalcDynamicLayout( PyObject *self, PyObject *args ) 
{
	...
	int length, dwMode;
	// @pyparm int|length||The requested dimension of the control bar, either horizontal or vertical, depending on dwMode.
	// @pyparm int|dwMode||A combination of flags.
	if (!PyArg_ParseTuple( args, "ii:CalcDynamicLayout", &length, &dwMode)) return NULL;
	GUI_BGN_SAVE;
...
PyObject *PyCControlBar_CalcFixedLayout( PyObject *self, PyObject *args ) 
{
	...
	if (!pCtlBar) return NULL;
	int stretch, horz;
	// @pyparm int|bStretch||Indicates whether the bar should be stretched to the size of the frame. The bStretch parameter is nonzero when the bar is not a docking bar (not available for docking) and is 0 when it is docked or floating (available for docking).
	// @pyparm int|bHorz||Indicates that the bar is horizontally or vertically oriented.
	if (!PyArg_ParseTuple( args, "ii:CalcFixedLayout", &stretch, &horz)) return NULL;
	GUI_BGN_SAVE;
...
PyObject *PyCControlBar_EraseNonClient( PyObject *self, PyObject *args ) 
{
	CControlBar *pCtlBar = PyCControlBar::GetControlBar(self);
	if (!pCtlBar) return NULL;
	if (!PyArg_ParseTuple( args, ":EraseNonClient")) return NULL;
	GUI_BGN_SAVE;
...
PyCControlBar_methods[] =
{
	...
	{"CalcFixedLayout", PyCControlBar_CalcFixedLayout,1},// @pymeth CalcFixedLayout|Calculates the horizontal size of a control bar
...
{
  return (CToolBar *)GetGoodCppObject( self, &type);
}
...
PyCToolBar_SetButtons(PyObject *self, PyObject *args)
{
	...
	if (PyInt_Check(buttons)) {
		// @pyparmalt1 int|numButtons||The number of buttons to pre-allocate.  If this option is used, then <om PyCToolBar.PySetButtonInfo> must be used.
...
static PyObject *
PyCToolBar_SetButtonInfo(PyObject *self, PyObject *args)
{
	CToolBar *pToolBar = PyCToolBar::GetToolBar(self);
	if (!pToolBar)return NULL;
...
static PyObject *
PyCToolBar_GetToolTips(PyObject *self, PyObject *args)
{
...
static PyObject *
PyCToolBar_SetBarStyle(PyObject *self, PyObject *args)
{
	CToolBar *pToolBar = PyCToolBar::GetToolBar(self);
	if (!pToolBar)return NULL;
	DWORD dwStyle;
...
PyCToolBar_methods[] =
{
	...
	{"GetToolBarCtrl",  PyCToolBar_GetToolBarCtrl,1}, // @pymeth GetToolBarCtrl|Returns the tool bar control object associated with the tool bar
	...
	{"SetButtonInfo",   PyCToolBar_SetButtonInfo,1},// @pymeth SetButtonInfo|Sets the button's command ID, style, and image number.
...
PyObject *
PyCToolBarCtrl_create(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
...
// @comm Userob is any Python object at all, but no reference count is kept, so you must ensure the object remains referenced throughout.
...
PyObject *PyCToolBarCtrl_AddButtons (PyObject *self, PyObject *args)
{
	...
	for (i = 0; i < numButtons; i++) {
		...
		if (!PyArg_ParseTuple (pButtonTuple, "iibbOi",
				       &btn[i].iBitmap,
				       &btn[i].idCommand,
				       &btn[i].fsState,
				       &btn[i].fsStyle,
...
// @pymethod int|PyCToolBarCtrl|CommandToIndex|Retrieve the zero-based index for the button associated with the specified command identifier.
...
static PyObject *
PyCToolBarCtrl_CreateWindow(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
// @pymethod <om PyCToolBarCtrl.TBBUTTON>|PyCToolBarCtrl|GetButton|Retrieve information about the specified button in a toolbar control.
...
// @pymethod left, top, right, bottom|PyCToolBarCtrl|GetItemRect|Retrieve the bounding rectangle of a button in a toolbar control.
...
PyObject *PyCToolBarCtrl_InsertButton (PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple(args,"i(iibbOi)",
			      &nID, // @pyparm int|nID||Zero-based index of a button. This function inserts the new button to the left of this button.
...
// @pymethod int|PyCToolBarCtrl|IsButtonIndeterminate|Determine whether the specified button in a toolbar control is indeterminate.
// @pyparm int|nID||ID of the button to check.
...
// @pymethod |PyCToolBarCtrl|SetCmdID|Set the command identifier which will be sent to the owner window when the specified button is pressed.
...
// @pymethod left, top, right, bottom|PyCToolBarCtrl|SetRows|Ask the toolbar control to resize itself to the requested number of rows.
PyObject *PyCToolBarCtrl_SetRows (PyObject *self, PyObject *args)
{
	...
	if (!PyArg_ParseTuple( args, "ii:SetRows",
			       ...
			       &bLarger)) // @pyparm int|bLarger||Tells whether to use more rows or fewer rows if the toolbar cannot be resized to the requested number of rows.
		return NULL;
...
PyCToolBarCtrl_methods[] =
{
	...
	{"CommandToIndex", PyCToolBarCtrl_CommandToIndex, 1}, // @pymeth CommandToIndex|Retrieve the zero-based index for the button associated with the specified command identifier.
	...
	{"EnableButton", PyCToolBarCtrl_EnableButton, 1}, // @pymeth EnableButton|Enable or disable a toolbar control button.
	{"GetBitmapFlags", PyCToolBarCtrl_GetBitmapFlags, 1}, // @pymeth GetBitmapFlags|Retrieve the bitmap flags from the toolbar.
	{"GetButton", PyCToolBarCtrl_GetButton, 1}, // @pymeth GetButton|Retrieve information about the specified button in a toolbar control.
	{"GetButtonCount", PyCToolBarCtrl_GetButtonCount, 1}, // @pymeth GetButtonCount|Retrieve a count of the buttons currently in the toolbar control.
	{"GetItemRect", PyCToolBarCtrl_GetItemRect, 1}, // @pymeth GetItemRect|Retrieve the bounding rectangle of a button in a toolbar control.
	{"GetRows", PyCToolBarCtrl_GetRows, 1}, // @pymeth GetRows|Retrieve the number of rows of buttons currently displayed
	{"HideButton", PyCToolBarCtrl_HideButton, 1}, // @pymeth HideButton|Hide or show the specified button in a toolbar control.
	{"Indeterminate", PyCToolBarCtrl_Indeterminate, 1}, // @pymeth Indeterminate|Hide or show the specified button in a toolbar control.
	...
	{"IsButtonIndeterminate", PyCToolBarCtrl_IsButtonIndeterminate, 1}, // @pymeth IsButtonIndeterminate|See if a button is Indeterminate.
	{"IsButtonPressed", PyCToolBarCtrl_IsButtonPressed, 1}, // @pymeth IsButtonPressed|See if a button is pressed.
	{"PressButton", PyCToolBarCtrl_PressButton, 1}, // @pymeth PressButton|Mark or unmark the specified button as pressed.
	{"SetBitmapSize", PyCToolBarCtrl_SetBitmapSize, 1}, // @pymeth SetBitmapSize|Set the size of the actual bitmapped images to be added to a toolbar control.
	{"SetButtonSize", PyCToolBarCtrl_SetButtonSize, 1}, // @pymeth SetButtonSize|Set the size of the actual buttons to be added to a toolbar control.
	{"SetCmdID", PyCToolBarCtrl_SetCmdID, 1}, // @pymeth SetCmdID|Set the command identifier which will be sent to the owner window when the specified button is pressed.
	{"SetRows", PyCToolBarCtrl_SetRows, 1}, // @pymeth SetRows|Ask the toolbar control to resize itself to the requested number of rows.
	{ NULL, NULL }
};

ui_type_CObject PyCToolBarCtrl::type ("PyCToolBarCtrl",
				      &PyCWnd::type,
...
//
/* static */ CStatusBar *
PyCStatusBar::GetStatusBar (PyObject *self)
{
...
PyObject *
PyCStatusBar::create (PyObject *self, PyObject *args)
{
  PyObject *parent;
