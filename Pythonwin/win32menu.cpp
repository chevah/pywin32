...
PyObject *PyCMenu::EnableMenuItem(PyObject *self, PyObject *args)
{
	...
	int id,flags;
	if (!PyArg_ParseTuple(args,"ii",
	                      &id, // @pyparm int|id||Specifies the command ID of the menu item. This parameter can specify pop-up menu items as well as standard menu items.
	                      &flags)) // @pyparm int|flags||Specifies the action to take. It can be a combination of MF_DISABLED, MF_ENABLED, or MF_GRAYED, with MF_BYCOMMAND or MF_BYPOSITION
		return NULL;
	GUI_BGN_SAVE;
