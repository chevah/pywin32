...
PyObject *
PyCSplitterWnd::create( PyObject *self, PyObject *args )
{
...
// splitter methods
//
CWnd *GetWnd(PyObject *self)
{
	return (CWnd *)PyCWnd::GetPythonGenericWnd(self);
}
CView *GetView(PyObject *self)
{
	return (CView *)PyCWnd::GetPythonGenericWnd(self);
}
...
PyObject *ui_splitter_create_static( PyObject *self, PyObject *args )
{
	...
	if (!PyArg_ParseTuple(args,"Oii|ii", 
			  ...
			  &nID))    // @pyparm int|id|AFX_IDW_PANE_FIRST|The child window ID of the window. The ID can be AFX_IDW_PANE_FIRST unless the splitter window is nested inside another splitter window.
		return NULL;
	if (!(ui_base_class::is_uiobject(ob, &PyCFrameWnd::type) ||
		ui_base_class::is_uiobject(ob, &PyCSplitterWnd::type)))
...
PyObject *ui_splitter_set_row_info( PyObject *self, PyObject *args )
{
	...
	if (!wnd)
		return NULL;
	int row, min, ideal;
	if (!PyArg_ParseTuple(args,"iii:SetRowInfo",
	          &row,    // @pyparm int|row||The row in the splitter.
...
PyObject *ui_splitter_set_column_info( PyObject *self, PyObject *args )
{
	...
	if (!wnd)
		return NULL;
	int row, min, ideal;
	if (!PyArg_ParseTuple(args,"iii:SetColumnInfo",
...
CPythonSplitter::CPythonSplitter()
{
	bHaveAllChildren = FALSE;
}
BOOL CPythonSplitter::CreateView(int row, int col,
...
void CPythonSplitter::OnSize( UINT nType, int cx, int cy )
{
	...
	if (!bHaveAllChildren) {
		...
		for (int col = 0; !bFailed && col < m_nCols; col++)
			for (int row = 0; !bFailed && row < m_nRows; row++)
				bFailed = (GetDlgItem(IdFromRowCol(row, col))==NULL);
