...
static PyObject *
PyCButton_get_bitmap(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CButton *pBut = GetButton(self);
	if (!pBut)
...
// @pymethod <o PyCProgressCtrl>|win32ui|CreateProgressCtrl|Creates a progress control object. <om PyProgressCtrl.Create> creates the actual control.
PyObject *
PyCProgressCtrl_create(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
...
PyObject *
PyCProgressCtrl_create_window(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
// @pymethod int|PyCProgressCtrl|OffsetPos|Advances the progress bar control's current position by the increment specified
static PyObject *
...
// @pymethod int|PyCProgressCtrl|StepIt|Advances the current position for a progress bar control by the step increment. Returns previous position.
static PyObject *
PyCProgressCtrl_step(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CProgressCtrl *pPC = GetProgressCtrl(self);
	if (!pPC)
...
// @object PyCProgressCtrl|A windows progress bar control.  Encapsulates an MFC <c CProgressCtrl> class.  Derived from <o PyCControl>.
static struct PyMethodDef PyCProgressCtrl_methods[] = {
	{"CreateWindow",    PyCProgressCtrl_create_window,1}, // @pymeth CreateWindow|Creates the window for a new progress bar object.
	{"SetRange",		PyCProgressCtrl_set_range,	1},	// @pymeth SetRange|Sets the lower and upper bounds for the progress bar.
	{"SetPos",			PyCProgressCtrl_set_pos,	1},	// @pymeth SetPos|Set the control's position
	{"OffsetPos",		PyCProgressCtrl_offset_pos,	1},	// @pymeth OffsetPos|Advances the progress bar control's current position by the increment specified.
	{"SetStep",			PyCProgressCtrl_set_step,	1},	// @pymeth SetStep|Specifies the step increment for a progress bar control.
	{"StepIt",			PyCProgressCtrl_step,		1},	// @pymeth StepIt|Advances the current position for a progress bar control by the step increment. Returns previous position.
	{NULL,				NULL}
};
...
PyObject *
PyCSliderCtrl_create_window(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
static PyObject *
PyCSliderCtrl_get_line_size(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_page_size(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_range_max(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_range_min(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_range(PyObject *self, PyObject *args)
{
	int nMin, nMax;
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
...
static PyObject *
PyCSliderCtrl_set_range_min(PyObject *self, PyObject *args)
{
	int nRangeMin;
	BOOL bRedraw = FALSE;
	if (!PyArg_ParseTuple(args, "i|i", 
...
static PyObject *
PyCSliderCtrl_set_range_max(PyObject *self, PyObject *args)
{
	int nRangeMax;
	BOOL bRedraw = FALSE;
	if (!PyArg_ParseTuple(args, "i|i", 
...
static PyObject *
PyCSliderCtrl_set_range(PyObject *self, PyObject *args)
{
	int nRangeMax;
	int nRangeMin;
	BOOL bRedraw = FALSE;
...
static PyObject *
PyCSliderCtrl_set_selection(PyObject *self, PyObject *args)
{
	int nRangeMax;
	int nRangeMin;
	if (!PyArg_ParseTuple(args, "ii", 
...
static PyObject *
PyCSliderCtrl_get_channel_rect(PyObject *self, PyObject *args)
{
	RECT rect;
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
...
static PyObject *
PyCSliderCtrl_get_thumb_rect(PyObject *self, PyObject *args)
{
	RECT rect;
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
...
static PyObject *
PyCSliderCtrl_get_pos(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_num_tics(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_get_tic_array(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
static PyObject *
PyCSliderCtrl_verify_pos(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
	CSliderCtrl *pSC = GetSliderCtrl(self);
	if (!pSC)
...
// @object PyCSliderCtrl|A windows Slider bar control.  Encapsulates an MFC <c CSliderCtrl> class.  Derived from <o PyCControl>.
static struct PyMethodDef PyCSliderCtrl_methods[] = {
	{"CreateWindow",    PyCSliderCtrl_create_window,1}, // @pymeth CreateWindow|Creates the window for a new Slider bar object.
	...
	{"GetTicPos",		PyCSliderCtrl_get_tic_pos,	1},	// @pymeth GetTicPos|Get the position of the specified tic in client coordinates
...
// @pymethod <o PyCStatusBarCtrl>|win32ui|CreateStatusBarCtrl|Creates a progress control object. <om PyStatusBarCtrl.Create> creates the actual control.
PyObject *
PyCStatusBarCtrl_create(PyObject *self, PyObject *args)
{
	CHECK_NO_ARGS(args);
...
PyObject *
PyCStatusBarCtrl_create_window(PyObject *self, PyObject *args)
{
	int style, id;
	PyObject *obParent;
	RECT rect;
...
// @pymethod (width, height, spacing)|PyCStatusBarCtrl|GetBorders|Retrieve the status bar control's current widths of the horizontal and vertical borders and of the space between rectangles.

static PyObject *
PyCStatusBarCtrl_get_borders(PyObject *self, PyObject *args)
{
...
static PyObject *
PyCStatusBarCtrl_get_parts(PyObject *self, PyObject *args)
{
	int i;
...
// @pymethod (left, top, right, bottom)|PyCStatusBarCtrl|GetRect|Retrieves the bounding rectangle of a part in a status bar control.

static PyObject *
PyCStatusBarCtrl_get_rect(PyObject *self, PyObject *args)
{
	int nPane;
	RECT rect;
	...
	if (!PyArg_ParseTuple(args,
			      "i:GetRect", // @pyparm int|nPane||Zero-based index of the part whose bounding rectangle is to be retrieved.
			      &nPane))
		return NULL;
...
PyObject *
PyCStatusBarCtrl_get_text (PyObject *self, PyObject *args)
{
	int attr;
	int nPane;
...
static PyObject *
PyCStatusBarCtrl_get_text_attr(PyObject *self, PyObject *args)
{
	int attr;
	int nPane;
...
static PyObject *
PyCStatusBarCtrl_get_text_length(PyObject *self, PyObject *args)
{
	int attr;
	int nPane;
...
static PyObject *
PyCStatusBarCtrl_set_min_height(PyObject *self, PyObject *args)
{
	int nHeight;
	CStatusBarCtrl *pSB = GetStatusBarCtrl(self);
...
// @pymethod |PyCStatusBarCtrl|SetParts|Sets the number of parts in a status bar control and the coordinate of the right edge of each part.
...
// @pymethod |PyCStatusBarCtrl|SetSimple|Specify whether a status bar control displays simple text or displays all control parts set by a previous call to SetParts.

static PyObject *
PyCStatusBarCtrl_set_simple (PyObject *self, PyObject *args)
{
	int bSimple;
	CStatusBarCtrl *pSB = GetStatusBarCtrl(self);
...
static struct PyMethodDef PyCStatusBarCtrl_methods[] = {
	...
	{"GetBorders", PyCStatusBarCtrl_get_borders, 1}, // @pymeth GetBorders|Retrieve the status bar control's current widths of the horizontal and vertical borders and of the space between rectangles.
	{"GetParts", PyCStatusBarCtrl_get_borders, 1}, // @pymeth GetParts|Retrieve coordinates of the parts in a status bar control.
	{"GetRect", PyCStatusBarCtrl_get_rect, 1}, // @pymeth GetRect|Retrieves the bounding rectangle of a part in a status bar control.
	{"GetText", PyCStatusBarCtrl_get_text, 1}, // @pymeth GetText|Retrieves the text of a part in a status bar control.
	{"GetTextAttr", PyCStatusBarCtrl_get_text_attr, 1}, // @pymeth GetTextAttr|Retrieves the text attributes of a part in a status bar control.
	{"GetTextLength", PyCStatusBarCtrl_get_text_length, 1}, // @pymeth GetTextLength|Retrieves the length of the text in a part in a status bar control.
...
PyObject *
PyCSpinButtonCtrl_GetPos(PyObject *self, PyObject *args)
{
...
PyObject *
PyCSpinButtonCtrl_SetPos(PyObject *self, PyObject *args)
{
	int pos;
...
PyObject *
PyCSpinButtonCtrl_SetRange(PyObject *self, PyObject *args)
{
	int min, max;
...
PyObject *
PyCSpinButtonCtrl_SetRange32(PyObject *self, PyObject *args)
{
	int min, max;
	if (!PyArg_ParseTuple(args, "ii", &min, &max))
		return NULL;
...
static struct PyMethodDef PyCSpinButtonCtrl_methods[] = {
	...
	{ "SetRange",           PyCSpinButtonCtrl_SetRange, 1}, // @pymeth SetRange|Sets the upper and lower limits (range) for a spin button control.
	{ "SetRange32",           PyCSpinButtonCtrl_SetRange32, 1}, // @pymeth SetRange32|Sets the upper and lower limits (range) for a spin button control.
	{ NULL }
};
