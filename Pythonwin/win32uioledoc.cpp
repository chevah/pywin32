...
// @pymethod |PyCOleDocument|EnableCompoundFile|Call this function if you want to store the document using the compound-file format.
...
// @pymethod <o PyCOleClientItem>|PyCOleDocument|GetInPlaceActiveItem|Obtains the OLE item that is currently activated in place in the frame window containing the view identified by obWnd.
...
static struct PyMethodDef PyCOleDocument_methods[] = {
	{"EnableCompoundFile", PyCOleDocument_EnableCompoundFile, 1}, // @pymeth EnableCompoundFile|Call this function if you want to store the document using the compound-file format
	{"GetStartPosition",   PyCOleDocument_GetStartPosition, 1}, // @pymeth GetStartPosition|Obtains the position of the first item in the document.
	{"GetNextItem",        PyCOleDocument_GetNextItem, 1}, // @pymeth GetNextItem|Call this function repeatedly to access each of the items in your document.
