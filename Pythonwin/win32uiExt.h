...
public:
	...
	// CPropertySheet.
	CPythonWndFramework(UINT nIDCaption, CWnd* pParentWnd, UINT iSelectPage) :
		T(nIDCaption, pParentWnd, iSelectPage) {;}
	CPythonWndFramework(LPCTSTR pszCaption, CWnd* pParentWnd, UINT iSelectPage) :
		T(pszCaption, pParentWnd, iSelectPage) {;}
	// CPropertyPage
	CPythonWndFramework(UINT id, UINT caption) :
		T(id, caption) {;}
	CPythonWndFramework(LPCTSTR id, UINT caption) :
		T(id, caption) {;}
	// CFormView
	CPythonWndFramework(UINT id) :
		T(id) {;}
	CPythonWndFramework(LPCTSTR id) :
		T(id) {;}
	// CCtrlView
	CPythonWndFramework(LPCTSTR lpszClass, DWORD dwStyle) :
		T(lpszClass, dwStyle) {;}
	// CPrintDialog.
	CPythonWndFramework(BOOL bPrintSetupOnly, DWORD dwFlags, CWnd* pParentWnd) :
		T(bPrintSetupOnly, dwFlags, pParentWnd) {;}
	// End of ctor hacks!!!!
	...
	BOOL _BasePreTranslateMessage(MSG *pMsg) {
	...
	BOOL _BaseOnCommand(WPARAM wparam, LPARAM lparam )
	{
		return T::OnCommand(wparam, lparam);
	}
	virtual BOOL PreCreateWindow(CREATESTRUCT &cs) {
		// @pyvirtual BOOL|PyCWnd|PreCreateWindow|Called by the framework before the creation of the Windows window attached to this View object.
	...
	afx_msg BOOL OnQueryNewPalette() {
		// @pyvirtual BOOL|PyCWnd|OnQueryNewPalette|Informs the window it is about to receive input focus, and shoudl realize its logical palette.
		...
		if (helper.HaveHandler()) {
			if (bCalcValidRects) {
				...
				if (pwp==NULL) {
					...
					obPos = Py_BuildValue("iiiiiii", pwp->hwnd, pwp->hwndInsertAfter, pwp->x, pwp->y, pwp->cx, pwp->cy, pwp->flags);
...
public:
	...
	// CPropertyPage
	CPythonDlgFramework(UINT id, UINT caption) :
...
{
public:
	CPythonPropertyPageFramework(UINT id, UINT caption) :
		CPythonDlgFramework<T>(id, caption) {;}
	CPythonPropertyPageFramework(LPCTSTR id, UINT caption) :
		CPythonDlgFramework<T>(id, caption) {;}
	...
	{
		// @pyvirtual int|PyCPropertyPage|OnWizardBack|Called by the framework when the user clicks on the Back button in a wizard.
		// @rdesc Return 0 to automatically advance to the next page;  -1 to prevent the page from changing. To jump to a page other than the next one, return the identifier of the dialog to be displayed.
		// @comm Note - If you provide a handler, you must call the underlying MFC method (<om PyCPropertyPage.OnWizardBack>) yourself
	...
	{
		// @pyvirtual int|PyCPropertyPage|OnWizardNext|Called by the framework when the user clicks on the Next button in a wizard.
		// @rdesc Return 0 to automatically advance to the next page;  -1 to prevent the page from changing. To jump to a page other than the next one, return the identifier of the dialog to be displayed.
		// @comm Note - If you provide a handler, you must call the underlying MFC method (<om PyCPropertyPage.OnWizardNext>) yourself
...
public:
	...
	// CCtrlView ctor
	CPythonViewFramework(LPCTSTR lpszClass, DWORD dwStyle) :
		CPythonWndFramework<T>(lpszClass, dwStyle) {;}
...
public:
	...
	virtual void OnCancel() {
		// @pyvirtual int|PyCPrintDialog|OnCancel|Called by the MFC architecture when the user selects the Cancel button.
		// @comm The procedure is expected to dismiss the window with the <om PyCPrintDialog.EndDialog> method.
		// The base implementation (which dismisses the dialog) is not called if a handler exists.  This can be
	...
	afx_msg void HandleCollate() {
		...
		int val = ((CButton*)ctl)->GetCheck();
                int useDevMode = (m_pd.Flags&PD_USEDEVMODECOPIES ||
                                  m_pd.Flags&PD_USEDEVMODECOPIESANDCOLLATE);
                LPDEVMODE devMode;
                if (useDevMode) {
	...
	afx_msg void HandlePrintRange(UINT nID) {
		CheckRadioButton(IDC_PRINT_RANGE_ALL,
