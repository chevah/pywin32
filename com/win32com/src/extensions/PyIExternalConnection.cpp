...
// @pymethod int|PyIExternalConnection|ReleaseConnection|Decrements an object's count of its strong external connections (references).
PyObject *PyIExternalConnection::ReleaseConnection(PyObject *self, PyObject *args)
{
	...
	// @pyparm int|fLastReleaseCloses||TRUE specifies that if the connection being released is the last external lock on the object, the object should close. FALSE specifies that the object should remain open until closed by the user or another process.
	DWORD extconn;
	DWORD reserved;
...
// @comm The IExternalConnection interface manages a server object's count of marshaled, or external, connections. A server that maintains such a count can detect when it has no external connections and shut itself down in an orderly fashion.
static struct PyMethodDef PyIExternalConnection_methods[] =
{
	{ "AddConnection", PyIExternalConnection::AddConnection, 1 }, // @pymeth AddConnection|Increments an object's count of its strong external connections (links).
	{ "ReleaseConnection", PyIExternalConnection::ReleaseConnection, 1 }, // @pymeth ReleaseConnection|Decrements an object's count of its strong external connections (references).
	{ NULL }
};
