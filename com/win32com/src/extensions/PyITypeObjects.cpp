...
void *AllocMore( void *pRoot, size_t size, BOOL bForVariant = FALSE )
{
	...
	if (pNewBlock==NULL) return PyErr_NoMemory();
...
void *AllocateMoreBuffer(size_t size)
{
	...
	if (pRoot==NULL) return PyErr_NoMemory();
	memset(pRoot, 0, size);
	pRoot->pNext = 0;
...
PyObject *PyObject_FromARRAYDESC(ARRAYDESC *ad)
{
	...
	{
		PyTuple_SetItem(ret, 1+i, Py_BuildValue("(ii)",
			ad->rgbounds[i].cElements,
			ad->rgbounds[i].lLbound));
	}
	return ret;
}
	...
	switch (pDesc->vt) {
		...
		case VT_USERDEFINED:
			if (!PyInt_Check(obExtra)) {
				PyErr_SetString(PyExc_TypeError, "If the TYPEDESC is of type VT_USERDEFINED, the object must be an integer");
				goto done;
			}
