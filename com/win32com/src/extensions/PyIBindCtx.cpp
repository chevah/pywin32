...
PyObject *PyWinObject_FromBIND_OPTS(BIND_OPTS *pbind_opts)
{
	return Py_BuildValue("{s:k,s:k,s:k,s:k}",
		"cbStruct",				pbind_opts->cbStruct,
		"Flags",				pbind_opts->grfFlags,
		"Mode",					pbind_opts->grfMode,
		"TickCountDeadline",	pbind_opts->dwTickCountDeadline);
}
...
static struct PyMethodDef PyIBindCtx_methods[] =
{
	...
	{"RegisterObjectParam",		PyIBindCtx::RegisterObjectParam,	METH_VARARGS, "Associates a COM object to the bind context"},
	// @pymeth RevokeObjectParam|Removes one of the bind context's associated objects
	{"RevokeObjectParam",		PyIBindCtx::RevokeObjectParam,		METH_VARARGS, "Removes one of the bind context's associated objects"},
