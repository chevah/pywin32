...
/*static*/ IClassFactory *PyIClassFactory::GetI(PyObject *self)
{
	return (IClassFactory *)PyIUnknown::GetI(self);
}
...
PyObject *PyIClassFactory::CreateInstance(PyObject *self, PyObject *args)
{
	...
	// @pyparm <o PyIUnknown>|outerUnknown||Usually None, otherwise the outer unknown if the object is being created as part of an aggregate.
	...
	if (pClassFactory==NULL) return NULL;
...
// @pymethod |PyIClassFactory|LockServer|Called by the client of a class object to keep a server open in memory, allowing instances to be created more quickly.
...
PyComTypeObject PyIClassFactory::type("PyIClassFactory",
                 &PyIUnknown::type,
                 sizeof(PyIClassFactory),
                 PyIClassFactory_methods,
				 GET_PYCOM_CTOR(PyIClassFactory));
