...
BOOL PythonOleArgHelper::MakeObjToVariant(PyObject *obj, VARIANT *var, PyObject *reqdObjectTuple)
{
	...
	case VT_BSTR:
		...
		else
		{
			...
			if ( !PyWinObject_AsBstr(obUse, &V_BSTR(var)) ) BREAK_FALSE
		}
		break;
	case VT_BSTR | VT_BYREF:
		if (bCreateBuffers)
		...
		*V_BSTRREF(var) = NULL;
		...
		if (!VALID_BYREF_MISSING(obj)) {
			if ( PyString_Check(obj) || PyUnicode_Check(obj) )
			{
				if ( !PyWinObject_AsBstr(obj, V_BSTRREF(var)) ) BREAK_FALSE
			}
			else
			{
				// Use str(object) instead!
...
#define MYBOOLREF (var->pboolVal)
	...
	case VT_DISPATCH | VT_BYREF:
		if (bCreateBuffers) {
			...
			if (*V_DISPATCHREF(var)) (*V_DISPATCHREF(var))->Release();
	...
	case VT_UNKNOWN | VT_BYREF:
		if (bCreateBuffers) {
			...
			if (*V_UNKNOWNREF(var)) (*V_UNKNOWNREF(var))->Release();
