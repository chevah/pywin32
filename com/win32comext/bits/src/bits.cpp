...
{
    // @object PyObject_FromBG_FILE_PROGRESS|A tuple of 3 elements (bytesTotal, bytesTransfered, completed), (int, int, bool)
    return Py_BuildValue("NNO",
                         PyLong_FromLongLong(fp->BytesTotal),
                         PyLong_FromLongLong(fp->BytesTransferred),
                         fp->Completed ? Py_True : Py_False);
}

PyObject *PyObject_FromBG_JOB_PROGRESS(BG_JOB_PROGRESS *jp)
{
    // @object PyObject_FromBG_JOB_PROGRESS|A tuple of 4 elements (bytesTotal, bytesTransfered, filesTotal, filesTransfered), all ints.
...
PyObject *PyObject_FromBG_JOB_TIMES(BG_JOB_TIMES *jt)
{
    ...
    return Py_BuildValue("NNN",
                         MakeTimeOrNone(jt->CreationTime),
                         MakeTimeOrNone(jt->ModificationTime),
...
// 	// cert store location
	...
	PyModule_AddIntConstant(module, "BG_ERROR_CONTEXT_QUEUE_MANAGER_NOTIFICATION", BG_ERROR_CONTEXT_QUEUE_MANAGER_NOTIFICATION);
