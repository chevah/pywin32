...
PyITransferAdviseSink::PyITransferAdviseSink(IUnknown *pdisp):
	PyIUnknown(pdisp)
{
	ob_type = &type;
}
...
PyObject *PyITransferAdviseSink::UpdateProgress(PyObject *self, PyObject *args)
{
	...
	int nFoldersCurrent;
	int nFoldersTotal;
	if ( !PyArg_ParseTuple(args, "KKiiii:UpdateProgress", &ullSizeCurrent, &ullSizeTotal,
		&nFilesCurrent, &nFilesTotal, &nFoldersCurrent, &nFoldersTotal) )
		return NULL;
	HRESULT hr;
	PY_INTERFACE_PRECALL;
	hr = pITAS->UpdateProgress( ullSizeCurrent, ullSizeTotal, nFilesCurrent, nFilesTotal, nFoldersCurrent, nFoldersTotal );
...
// @pymethod int|PyITransferAdviseSink|ConfirmEncryptionLoss|Notifies user when an item can't be encrypted at destination
...
// @pymethod int|PyITransferAdviseSink|SubStreamFailure|Notifies user of failure on a substream, and queries how to proceed
...
// @object PyITransferAdviseSink|Interface that receives notifications from <o PyITransferSource> or <o PyITransferDestination>
static struct PyMethodDef PyITransferAdviseSink_methods[] =
{
	{ "UpdateProgress", PyITransferAdviseSink::UpdateProgress, 1 }, // @pymeth UpdateProgress|Gives an estimate of amount of work completed
	{ "UpdateTransferState", PyITransferAdviseSink::UpdateTransferState, 1 }, // @pymeth UpdateTransferState|Notifies client of current operation state
	{ "ConfirmOverwrite", PyITransferAdviseSink::ConfirmOverwrite, 1 }, // @pymeth ConfirmOverwrite|Asks user for permission to overwrite an existing item
	{ "ConfirmEncryptionLoss", PyITransferAdviseSink::ConfirmEncryptionLoss, 1 }, // @pymeth ConfirmEncryptionLoss|Notifies user when an item can't be encrypted at destination
	{ "FileFailure", PyITransferAdviseSink::FileFailure, 1 }, // @pymeth FileFailure|Notifies user of failure, and queries how to proceed
	{ "SubStreamFailure", PyITransferAdviseSink::SubStreamFailure, 1 }, // @pymeth SubStreamFailure|Notifies user of failure on a substream, and queries how to proceed
	{ "PropertyFailure", PyITransferAdviseSink::PropertyFailure, 1 }, // @pymeth PropertyFailure|Notifies user of failure to set an item's properties
	{ NULL }
};

PyComTypeObject PyITransferAdviseSink::type("PyITransferAdviseSink",
		&PyIUnknown::type,
		sizeof(PyITransferAdviseSink),
		PyITransferAdviseSink_methods,
		GET_PYCOM_CTOR(PyITransferAdviseSink));
// ---------------------------------------------------
//
...
{
	PY_GATEWAY_METHOD;
	HRESULT hr=InvokeViaPolicy("UpdateProgress", NULL, "KKiiii", ullSizeCurrent, ullSizeTotal, nFilesCurrent, nFilesTotal, nFoldersCurrent, nFoldersTotal);
	return hr;
}
		...
		/* [in] */ HRESULT hrError)
{
	...
	if (obkey==NULL) return MAKE_PYCOM_GATEWAY_FAILURE_CODE("PropertyFailure");
