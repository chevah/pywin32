...
IShellLinkDataList *PyIShellLinkDataList::GetI(PyObject *self)
{
	return (IShellLinkDataList *)PyIUnknown::GetI(self);
}
...
{	static char *NT_CONSOLE_PROPS_keywords[]={
		"Signature","FillAttribute","PopupFillAttribute","ScreenBufferSize","WindowSize",
		"WindowOrigin","Font","InputBufferSize","FontSize","FontFamily","FontWeight","FaceName",
		"CursorSize","FullScreen","QuickEdit","InsertMode","AutoPosition","HistoryBufferSize",
		"NumberOfHistoryBuffers","HistoryNoDup","ColorTable", "Size", NULL};
	...
	switch(sig){
		case NT_CONSOLE_PROPS_SIG:{
			...
			bsuccess=PyArg_ParseTupleAndKeywords(dummy_args, obdb, 
				...
				PyObject_AsCOORD, &buf->dwScreenBufferSize,	// @prop (int,int)|ScreenBufferSize|Size of console screen buffer, in character cells
				PyObject_AsCOORD, &buf->dwWindowSize,	// @prop (int,int)|WindowSize|Size of console window in character cells
				PyObject_AsCOORD, &buf->dwWindowOrigin,	// @prop (int,int)|WindowOrigin|Window position, in screen coordinates
				&buf->nFont, // @prop int|nFont|Number of font to be displayed.  See <om win32console.GetNumberOfConsoleFonts>
				...
				&buf->uCursorSize,	// @prop int|CursorSize|Relative size of cursor, expressed as percent of character size
...
PyObject *PyObject_FromDATABLOCK(void *datablock)
{
	...
	switch(sig){
		case NT_CONSOLE_PROPS_SIG:{
			...
			return Py_BuildValue("{s:k,s:k,s:H,s:H,s:(HH),s:(HH),s:(HH),s:k,s:k,s:(HH),s:k,s:k,s:N,s:k,s:O&,s:O&,s:O&,s:O&,s:k,s:k,s:O&,s:(kkkkkkkkkkkkkkkk)}",
				"Size", buf->dbh.cbSize,
				"Signature", buf->dbh.dwSignature,
				"FillAttribute", buf->wFillAttribute,
				"PopupFillAttribute", buf->wPopupFillAttribute,
				"ScreenBufferSize", buf->dwScreenBufferSize.X, buf->dwScreenBufferSize.Y,
				"WindowSize", buf->dwWindowSize.X, buf->dwWindowSize.Y,
				"WindowOrigin", buf->dwWindowOrigin.X, buf->dwWindowOrigin.Y,
				"Font", buf->nFont,
				"InputBufferSize", buf->nInputBufferSize,
				"FontSize", buf->dwFontSize.X, buf->dwFontSize.Y,
				"FontFamily", buf->uFontFamily,
				"FontWeight", buf->uFontWeight,
				"FaceName", (buf->FaceName[LF_FACESIZE-1]==L'\0') ? PyWinObject_FromWCHAR(buf->FaceName):PyWinObject_FromWCHAR(buf->FaceName, LF_FACESIZE),
				"CursorSize", buf->uCursorSize,
				"FullScreen", PyBool_FromLong, buf->bFullScreen,
				"QuickEdit", PyBool_FromLong, buf->bQuickEdit,
				"InsertMode", PyBool_FromLong, buf->bInsertMode,
				"AutoPosition", PyBool_FromLong, buf->bAutoPosition,
				"HistoryBufferSize", buf->uHistoryBufferSize,
				"NumberOfHistoryBuffers", buf->uNumberOfHistoryBuffers,
				"HistoryNoDup", PyBool_FromLong, buf->bHistoryNoDup,
				"ColorTable", buf->ColorTable[0], buf->ColorTable[1], buf->ColorTable[2], buf->ColorTable[3],
							  buf->ColorTable[4], buf->ColorTable[5], buf->ColorTable[6], buf->ColorTable[7],
							  buf->ColorTable[8], buf->ColorTable[9], buf->ColorTable[10], buf->ColorTable[11],
							  buf->ColorTable[12], buf->ColorTable[13], buf->ColorTable[14], buf->ColorTable[15]);
			}
		case NT_FE_CONSOLE_PROPS_SIG:{
		...
		case EXP_SPECIAL_FOLDER_SIG:{
			...
			return Py_BuildValue("{s:k,s:k,s:k,s:k}",
				"Size", buf->cbSize,
				"Signature", buf->dwSignature,
				"idSpecialFolder", buf->idSpecialFolder,
				"Offset", buf->cbOffset);
			}
		case EXP_DARWIN_ID_SIG:{
			EXP_DARWIN_LINK *buf=(EXP_DARWIN_LINK *)datablock;
			return Py_BuildValue("{s:k,s:k,s:s,s:u}",
				"Size", buf->dbh.cbSize,
				"Signature", buf->dbh.dwSignature,
				"DarwinID", buf->szDarwinID,
				"wDarwinID", buf->szwDarwinID);
			}
		case EXP_SZ_ICON_SIG:
		case EXP_SZ_LINK_SIG:{
...
static struct PyMethodDef PyIShellLinkDataList_methods[] =
{
	...
	{ "CopyDataBlock", PyIShellLinkDataList::CopyDataBlock, METH_VARARGS, "Retrieves the specified data block from the link"},
	...
	{ "SetFlags", PyIShellLinkDataList::SetFlags, METH_VARARGS, "Sets the flags indicating which data blocks are present" },
	{ NULL }
};

PyComTypeObject PyIShellLinkDataList::type("PyIShellLinkDataList",
		&PyIUnknown::type,
		sizeof(PyIShellLinkDataList),
		PyIShellLinkDataList_methods,
		GET_PYCOM_CTOR(PyIShellLinkDataList));
