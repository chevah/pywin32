...
/* static */ IViewObject *PyIViewObject::GetI(PyObject *self)
{
	return (IViewObject *)PyIUnknown::GetI(self);
}
...
PyComTypeObject PyIViewObject::type("PyIViewObject",
		&PyIUnknown::type,
		sizeof(PyIViewObject),
		PyIViewObject_methods,
		...
		/* [in] */ ULONG_PTR dwContinue)
{
	...
	PyObject *oblprcBounds = Py_BuildValue("llll", lprcBounds->left,lprcBounds->top, lprcBounds->right, lprcBounds->bottom);
...
/**
	...
	PyObject *result;
	HRESULT hr=InvokeViaPolicy("GetColorSet", &result, "iiOOi", dwDrawAspect, lindex, obpvAspect, obptd, hicTargetDev);
		...
		/* [out] */ DWORD __RPC_FAR * pdwFreeze)
{
	...
	if (!PyArg_Parse(result, "i" , pdwFreeze)) return PyCom_HandlePythonFailureToCOM(/*pexcepinfo*/);
	Py_DECREF(result);
	return hr;
}
...
{
	PY_GATEWAY_METHOD;
	if (ppAdvSink==NULL) return E_POINTER;
	PyObject *result;
	HRESULT hr=InvokeViaPolicy("GetAdvise", &result);
	if (FAILED(hr)) return hr;
	// Process the Python results, and convert back to the real params
	PyObject *obppAdvSink;
	if (!PyArg_ParseTuple(result, "iiO" , pAspects, pAdvf, &obppAdvSink)) return PyCom_HandlePythonFailureToCOM(/*pexcepinfo*/);
	BOOL bPythonIsHappy = TRUE;
