...
/* static */ IOleControl *PyIOleControl::GetI(PyObject *self)
{
	return (IOleControl *)PyIUnknown::GetI(self);
}
...
static struct PyMethodDef PyIOleControl_methods[] =
{
	...
	{ "OnAmbientPropertyChange", PyIOleControl::OnAmbientPropertyChange, 1 }, // @pymeth OnAmbientPropertyChange|Description of OnAmbientPropertyChange
	{ "FreezeEvents", PyIOleControl::FreezeEvents, 1 }, // @pymeth FreezeEvents|Description of FreezeEvents
	{ NULL }
};

PyComTypeObject PyIOleControl::type("PyIOleControl",
		&PyIUnknown::type,
		sizeof(PyIOleControl),
		PyIOleControl_methods,
...
{
	PY_GATEWAY_METHOD;
	PyObject *obMsg = Py_BuildValue("((iiiii(ii)))",pMsg->hwnd,pMsg->message,pMsg->wParam,pMsg->lParam,pMsg->time,pMsg->pt.x,pMsg->pt.y);
	if (obMsg==NULL) return PyCom_HandlePythonFailureToCOM();
