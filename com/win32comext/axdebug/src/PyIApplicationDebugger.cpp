...
PyObject *PyIApplicationDebugger::CreateInstanceAtDebugger(PyObject *self, PyObject *args)
{
	...
	if (!bPythonIsHappy) {
		if (pUnkOuter) pUnkOuter->Release();
		return NULL;
	}
	PY_INTERFACE_PRECALL;
...
PyObject *PyIApplicationDebugger::onHandleBreakPoint(PyObject *self, PyObject *args)
{
	...
	if ( !PyArg_ParseTuple(args, "OiO:onHandleBreakPoint", &obprpt, &br, &obError) )
		return NULL;
	BOOL bPythonIsHappy = TRUE;
	if (!PyCom_InterfaceFromPyInstanceOrObject(obprpt, IID_IRemoteDebugApplicationThread, (void **)&prpt, FALSE /* bNoneOK */))
		 bPythonIsHappy = FALSE;
	if (!PyCom_InterfaceFromPyInstanceOrObject(obError, IID_IActiveScriptErrorDebug, (void **)&pError, TRUE /* bNoneOK */))
		 bPythonIsHappy = FALSE;
	if (!bPythonIsHappy) return NULL;
	PY_INTERFACE_PRECALL;
...
PyObject *PyIApplicationDebugger::onDebuggerEvent(PyObject *self, PyObject *args)
{
	...
	IID iid;
	IUnknown *punk;
	if (!PyWinObject_AsIID(obIID, &iid)) bPythonIsHappy = FALSE;
...
static struct PyMethodDef PyIApplicationDebugger_methods[] =
{
	...
	{ "CreateInstanceAtDebugger", PyIApplicationDebugger::CreateInstanceAtDebugger, 1 }, // @pymeth CreateInstanceAtDebugger|Create objects in the application process address space.
	{ "onDebugOutput", PyIApplicationDebugger::onDebugOutput, 1 }, // @pymeth onDebugOutput|Called when <om PyIDebugApplication.DebugOutput> is called.
	{ "onHandleBreakPoint", PyIApplicationDebugger::onHandleBreakPoint, 1 }, // @pymeth onHandleBreakPoint|Called when a breakpoint is hit.
	{ "onClose", PyIApplicationDebugger::onClose, 1 }, // @pymeth onClose|Called when <om PyIDebugApplication.Close> is called.
	{ "onDebuggerEvent", PyIApplicationDebugger::onDebuggerEvent, 1 }, // @pymeth onDebuggerEvent|Handle a custom event.
	{ NULL }
};

PyComTypeObject PyIApplicationDebugger::type("PyIApplicationDebugger",
		&PyIUnknown::type,
		sizeof(PyIApplicationDebugger),
		PyIApplicationDebugger_methods,
		GET_PYCOM_CTOR(PyIApplicationDebugger));
// ---------------------------------------------------
//
// Gateway Implementation

STDMETHODIMP PyGApplicationDebugger::QueryAlive(
		void)
{
	PY_GATEWAY_METHOD;
		...
		/* [iid_is][out] */ IUnknown __RPC_FAR *__RPC_FAR * ppvObject)
{
	...
	PyObject *result;
	HRESULT hr=InvokeViaPolicy("CreateInstanceAtDebugger", &result, "OOiO", obrclsid, obpUnkOuter, dwClsContext, obriid);
	Py_XDECREF(obrclsid);
...
STDMETHODIMP PyGApplicationDebugger::onClose(
		void)
{
	PY_GATEWAY_METHOD;
...
STDMETHODIMP PyGApplicationDebugger::onDebuggerEvent(
		REFIID riid,
		IUnknown *punk)
{
	PY_GATEWAY_METHOD;
