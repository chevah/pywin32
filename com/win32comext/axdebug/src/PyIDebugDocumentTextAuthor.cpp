...
// @object PyIDebugDocumentTextAuthor|This preliminary interface is provided by text documents that support editing. Derived from <o PyIDebugDocumentText>
...
PyComTypeObject PyIDebugDocumentTextAuthor::type("PyIDebugDocumentTextAuthor",
		&PyIDebugDocumentText::type,
		sizeof(PyIDebugDocumentTextAuthor),
		PyIDebugDocumentTextAuthor_methods,
...
// Std delegation
// IDebugDocumentInfo
STDMETHODIMP PyGDebugDocumentTextAuthor::GetName(DOCUMENTNAMETYPE dnt, BSTR *pbstrName) {return PyGDebugDocumentText::GetName(dnt, pbstrName);}
STDMETHODIMP PyGDebugDocumentTextAuthor::GetDocumentClassId(CLSID *pclsidDocument) {return PyGDebugDocumentText::GetDocumentClassId(pclsidDocument);}

// IDebugDocument
STDMETHODIMP PyGDebugDocumentTextAuthor::GetDocumentAttributes(TEXT_DOC_ATTR __RPC_FAR * ptextdocattr) {return PyGDebugDocumentText::GetDocumentAttributes(ptextdocattr);}

STDMETHODIMP PyGDebugDocumentTextAuthor::GetSize(ULONG __RPC_FAR * pcNumLines, ULONG __RPC_FAR * pcNumChars){return PyGDebugDocumentText::GetSize(pcNumLines, pcNumChars);}
STDMETHODIMP PyGDebugDocumentTextAuthor::GetPositionOfLine(ULONG cLineNumber, ULONG __RPC_FAR * pcCharacterPosition){return PyGDebugDocumentText::GetPositionOfLine(cLineNumber, pcCharacterPosition);}
STDMETHODIMP PyGDebugDocumentTextAuthor::GetLineOfPosition(ULONG cCharacterPosition,ULONG __RPC_FAR * pcLineNumber,ULONG __RPC_FAR * pcCharacterOffsetInLine){return PyGDebugDocumentText::GetLineOfPosition(cCharacterPosition, pcLineNumber, pcCharacterOffsetInLine);}
STDMETHODIMP PyGDebugDocumentTextAuthor::GetText(ULONG cCharacterPosition,WCHAR __RPC_FAR * pcharText,SOURCE_TEXT_ATTR __RPC_FAR * pstaTextAttr,ULONG __RPC_FAR * pcNumChars,ULONG cMaxChars) {return PyGDebugDocumentText::GetText(cCharacterPosition,pcharText,pstaTextAttr,pcNumChars,cMaxChars);}

STDMETHODIMP PyGDebugDocumentTextAuthor::GetPositionOfContext(IDebugDocumentContext __RPC_FAR * psc, ULONG __RPC_FAR * pcCharacterPosition, ULONG __RPC_FAR * cNumChars) {return PyGDebugDocumentText::GetPositionOfContext(psc, pcCharacterPosition, cNumChars);}
STDMETHODIMP PyGDebugDocumentTextAuthor::GetContextOfPosition(ULONG cCharacterPosition,ULONG cNumChars,IDebugDocumentContext __RPC_FAR *__RPC_FAR * ppsc){return PyGDebugDocumentText::GetContextOfPosition(cCharacterPosition, cNumChars, ppsc);}
