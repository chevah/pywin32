...
PyIInitializeWithFile::PyIInitializeWithFile(IUnknown *pdisp):
	PyIUnknown(pdisp)
{
	ob_type = &type;
}
...
{
	{ "Initialize", PyIInitializeWithFile::Initialize, 1 }, // @pymeth Initialize|Passes a file path to a property handler on startup
	{ NULL }
};

PyComTypeObject PyIInitializeWithFile::type("PyIInitializeWithFile",
		&PyIUnknown::type,
		sizeof(PyIInitializeWithFile),
		PyIInitializeWithFile_methods,
		GET_PYCOM_CTOR(PyIInitializeWithFile));
// ---------------------------------------------------
//
