...
PyIPropertyStore::PyIPropertyStore(IUnknown *pdisp):
	PyIUnknown(pdisp)
{
	ob_type = &type;
}
...
/* static */ IPropertyStore *PyIPropertyStore::GetI(PyObject *self)
{
	return (IPropertyStore *)PyIUnknown::GetI(self);
}
...
PyObject *PyIPropertyStore::SetValue(PyObject *self, PyObject *args)
{
	...
	// @pyparm <o PyPROPVARIANT>|Value||Variant value which can be converted to the appropriate variant type for the property
	// Pass a VT_EMPTY variant to indicate that the property should be removed.
	if (!PyArg_ParseTuple(args, "O&O&:SetValue",
...
PyComTypeObject PyIPropertyStore::type("PyIPropertyStore",
		&PyIUnknown::type,
		sizeof(PyIPropertyStore),
		PyIPropertyStore_methods,
		GET_PYCOM_CTOR(PyIPropertyStore));
...
STDMETHODIMP PyGPropertyStore::GetAt(
	DWORD iProp,
	PROPERTYKEY *pkey)
{
	PY_GATEWAY_METHOD;
...
STDMETHODIMP PyGPropertyStore::GetValue(
	REFPROPERTYKEY key,
	PROPVARIANT * pv)
{
	PY_GATEWAY_METHOD;
	PyObject *obkey = PyWinObject_FromPROPERTYKEY(key);
	if (obkey==NULL) return MAKE_PYCOM_GATEWAY_FAILURE_CODE("GetValue");
	PyObject *result;
...
STDMETHODIMP PyGPropertyStore::SetValue(
	REFPROPERTYKEY key,
	REFPROPVARIANT propvar)
{
	PY_GATEWAY_METHOD;
	PyObject *obkey = PyWinObject_FromPROPERTYKEY(key);
	if (obkey==NULL) return MAKE_PYCOM_GATEWAY_FAILURE_CODE("SetValue");
